...
Previous checkpoint: v0.6.308 Safe Local-Only Runnable Demo Path Candidate  
Reviewed candidate: `safe_local_only_runnable_demo_path_candidate_v06308`  
Decision result: accepted as safe local-only runnable demo path candidate  
...
This checkpoint reviews and accepts the v0.6.308 Safe Local-Only Runnable Demo Path Candidate.

The candidate is accepted as the basis for a future safe local-only runnable demo path definition. It remains review-only. It does not create a runnable demo path, implement concrete checks, apply runtime enforcement, authorize execution, permit real execution, or change gateway/runtime behavior.
...
| entrypoint | accepted as future reviewer-facing path | not runnable demo creation |
| target lab profile | accepted | no private LAN/public/customer target access |
| runtime destination binding | accepted | not external target authorization |
| preflight sequence | accepted | preflight not satisfied yet |
| execution authorization gate | accepted as required gate | execution authorization remains false |
| human approval gate | accepted as required gate | no automatic approval |
| mock-first execution | accepted | not scanner readiness |
...
~~~text
safe local-only runnable demo path candidate prerequisites are accepted
safe local-only runnable demo path candidate entrypoint is accepted
safe local-only runnable demo path candidate target lab profile is accepted
safe local-only runnable demo path candidate runtime destination binding is accepted
safe local-only runnable demo path candidate preflight sequence is accepted
safe local-only runnable demo path candidate execution authorization gate is accepted
safe local-only runnable demo path candidate evidence output is accepted
safe local-only runnable demo path candidate expected outputs are accepted
...
- target lab profile evidence expectation
- runtime destination binding evidence expectation
- preflight sequence and preflight evidence expectation
...
- result/evidence/review records remain under private-not-in-git
