...
Previous checkpoint: v0.6.303 Next Work Selection Using Risk-Tiered Granularity  
Candidate result: safe local-only demo execution boundary candidate created  
Application status: candidate only; no boundary applied, runnable demo path created, execution authorization, runtime readiness, scanner readiness, or gateway behavior changed

## Purpose

This checkpoint creates a Safe Local-Only Demo Execution Boundary Candidate.

The candidate defines how a future local-only runnable demo should be constrained before any runnable path is created. It keeps execution blocked and treats `localhost_only` as a candidate boundary, not as authorization.
...
## Boundary candidate matrix

| boundary candidate area | candidate rule | still not granted |
| --- | --- | --- |
| target mode | `localhost_only` | external target authorization |
| allowed target candidates | loopback-only names and addresses such as localhost, 127.0.0.1, and ::1 | private LAN or public internet targets |
| denied target candidates | DNS names outside localhost, public IPs, RFC1918 LAN targets, customer/third-party targets, production targets | any exception by AI request |
| tool mode | mock-first; no live scanner by default | scanner readiness |
| preflight | explicit target, binding, operator review, human approval, and evidence expectations must be satisfied later | runtime execution |
| fail-closed | ambiguous target, external discovery, credential use, missing authorization, or missing evidence blocks transition | real execution permission |
| evidence output | private-not-in-git generated records only | public movement of private generated outputs |

## Defined candidate assertions

~~~text
safe local-only demo execution boundary target mode candidate is localhost_only
safe local-only demo execution boundary loopback targets candidate is defined
safe local-only demo execution boundary external targets candidate is blocked
safe local-only demo execution boundary private LAN targets candidate is blocked
safe local-only demo execution boundary DNS targets candidate is blocked
safe local-only demo execution boundary preflight requirements candidate is defined
safe local-only demo execution boundary fail-closed conditions candidate is defined
safe local-only demo execution boundary evidence outputs candidate is defined
safe local-only demo execution boundary candidate excludes external target authorization
safe local-only demo execution boundary candidate excludes real scanner execution
safe local-only demo execution boundary candidate excludes customer or third-party target
~~~

## Candidate target boundary

Allowed target candidates for a future local-only demo are limited to loopback-only identifiers:

- `localhost`
- `127.0.0.1`
- `::1`
- explicit loopback-bound container or service aliases only if later bound to localhost-only behavior by review

Denied target candidates include:

- public internet hosts
- public IP addresses
- private LAN targets such as RFC1918 networks
- DNS names other than localhost unless later bound to loopback-only behavior by review
- customer or third-party targets
- production targets
- targets discovered by external discovery
- any target selected solely by AI rationale

## Candidate preflight and fail-closed requirements

A future runnable demo path should remain blocked unless later review creates explicit preflight checks for:
...
- operator review has been recorded
- human approval requirement is explicit
- evidence output location remains under `private-not-in-git`
- execution authorization is separately recorded and still defaults to false

Fail-closed conditions should include ambiguous targets, non-loopback targets, DNS uncertainty, credential use, external discovery, missing preflight evidence, missing operator review, missing human approval, missing runtime binding, missing execution authorization, or any AI-only justification.
