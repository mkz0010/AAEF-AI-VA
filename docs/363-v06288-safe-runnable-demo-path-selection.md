...
Status: selected demo path checkpoint  
Scope: AAEF-AI-VA safe demo path planning  
Previous checkpoint: v0.6.287 Safe Runnable Demo Gap Inventory  
Selection result: `safe_mock_demo_initial_path`  
Application status: selection only; no runtime demo readiness, scanner readiness, execution authorization, publication approval, or gateway behavior changed

## Purpose

This checkpoint selects the safest near-term runnable demo path after the v0.6.287 Safe Runnable Demo Gap Inventory.
...
The safe mock demo is selected as the initial path because it is currently runnable through private generated artifacts while local-only runtime execution remains blocked. This selection does not approve publication, does not move generated outputs public, does not authorize execution, and does not make scanner readiness or production readiness claims.

No private generated outputs are moved public in v0.6.288.

## Path selection matrix

| path candidate | current status | selection decision | reason |
| --- | --- | --- | --- |
| `safe_mock_demo_initial_path` | runnable through private generated artifacts | selected | demonstrates request / gate / evidence boundaries without real scanner execution |
| `local_only_runnable_demo` | gap inventory only, not ready | deferred | requires local-only execution boundary, runtime readiness, preflight satisfaction, and authorization gates |
| `real_scanner_execution` | blocked | rejected for current path | must remain blocked until a later, scoped, localhost-only boundary is accepted |

## Current selected path

The selected path uses the existing safe mock demo behavior:
...
## Deferred local-only runtime path

The local-only runnable demo remains a later boundary candidate.
...
This checkpoint selects a demo path only. It does not create a local-only demo execution boundary candidate, runtime demo readiness, scanner readiness, production readiness, execution authorization, real execution permission, public artifact promotion, publication approval, safe mock demo hardening candidate, gateway-path verification report, gateway behavior change, adapter behavior change, schema change, runtime behavior change, scanner behavior change, fixture creation, actual record creation, README front-page rewrite, repository metadata change, public announcement, or commercial terms.
