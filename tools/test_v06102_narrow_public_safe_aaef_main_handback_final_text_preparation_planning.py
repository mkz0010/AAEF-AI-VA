...
DOC = ROOT / "docs/178-v06102-narrow-public-safe-aaef-main-handback-final-text-preparation-planning.md"
...
def main() -> int:
    for path in [DOC, V06101_DOC, V06100_DOC, V0655_DOC, INDEX, VALIDATOR]:
    ...
    for phrase in required_phrases + REQUIRED_FLAGS:
        require(phrase in doc_text, f"v0.6.102 document missing required text: {phrase}")

    for category in EXPECTED_CATEGORIES:
        require(category in doc_text, f"v0.6.102 document missing retained category: {category}")
        ...
        require("expected_failure_category" not in metadata, f"v0.6.102 must not add metadata-level failure category: {category}")
    ...
    for phrase in [
        ...
        require(phrase in v06101, f"v0.6.101 baseline missing required phrase: {phrase}")

    v06100 = V06100_DOC.read_text(encoding="utf-8")
    require("narrow_public_safe_aaef_main_handback_text_drafting_candidate_close_ready = true" in v06100, "v0.6.100 text candidate must be close-ready")
    ...
    for phrase in [
        ...
        require(phrase not in lower_doc, f"v0.6.102 document must not include affirmative overclaim phrase: {phrase}")

    print("v0.6.102 narrow public-safe AAEF main handback final text preparation planning tests passed.")
    return 0
