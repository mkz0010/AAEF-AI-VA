...
DOC = ROOT / "docs/172-v0696-narrow-public-safe-aaef-main-handback-material-preparation-candidate-review-close-readiness.md"
V0695_DOC = ROOT / "docs/171-v0695-narrow-public-safe-aaef-main-handback-material-preparation-candidate.md"
V0694_DOC = ROOT / "docs/170-v0694-narrow-public-safe-aaef-main-handback-material-preparation-planning.md"
...
REQUIRED_DOC_FLAGS = [
    ...
    "close_narrow_public_safe_aaef_main_handback_material_preparation_candidate = true",
    "retain_v0695_narrow_public_safe_aaef_main_handback_material_preparation_candidate = true",
    "narrow_public_safe_aaef_main_handback_material_preparation_candidate_retained = true",
    "narrow_public_safe_aaef_main_handback_material_preparation_candidate_reviewed = true",
    "narrow_public_safe_aaef_main_handback_material_preparation_candidate_scope_preserved = true",
    "narrow_public_safe_aaef_main_handback_material_preparation_candidate_revision_required = false",
    "narrow_public_safe_aaef_main_handback_material_preparation_candidate_replaced = false",
    "narrow_public_safe_aaef_main_handback_material_preparation_candidate_uses_v0695_candidate = true",
    ...
    "narrow_public_safe_aaef_main_handback_material_preparation_candidate_reviewer_aid_retained = true",
    ...
    "narrow_public_safe_aaef_main_handback_material_candidate_close_ready = true",
    ...
    "aaef_main_handback_material_preparation_close_ready = true",
...
def main() -> int:
    require(V0695_DOC.exists(), "v0.6.95 material preparation candidate document must exist")
    require(V0694_DOC.exists(), "v0.6.94 material preparation planning document must exist")
    ...
    for flag in REQUIRED_DOC_FLAGS:
        require(flag in doc_text, f"v0.6.96 document missing close-readiness flag: {flag}")

    for phrase in [
        "Retained material preparation candidate scope",
        "Close-readiness checks",
        "Retained two-layer publication boundary",
        "Public/private two-layer boundary",
        "Public repository as trust proof/evaluation package",
        "Paid or NDA package as implementation/adoption package",
        ...
        "Candidate avoids production-readiness, diagnostic-completeness, certification, compliance, legal, audit, and equivalence claims",
    ]:
        require(phrase in doc_text, f"v0.6.96 document missing close-readiness/two-layer phrase: {phrase}")
    ...
    for phrase in [
        ...
        require(phrase in doc_text, f"v0.6.96 document missing retained lesson or boundary phrase: {phrase}")
    ...
    for phrase in [
        ...
        require(phrase in doc_text, f"v0.6.96 document missing deferred external review item: {phrase}")

    for category in EXPECTED_CATEGORIES:
        require(category in doc_text, f"v0.6.96 document missing retained category: {category}")
        require((FIXTURE_ROOT / category).exists(), f"fixture directory missing: {category}")

    for field in REQUIRED_METADATA_FIELDS:
        require(field in doc_text, f"v0.6.96 document missing retained metadata field: {field}")

    for flag in REQUIRED_BOUNDARY_FLAGS:
        require(f"{flag} = false" in doc_text, f"v0.6.96 document missing retained false boundary flag: {flag}")
    ...
    for phrase in [
        ...
        require(phrase in v0695, f"v0.6.95 baseline missing required phrase: {phrase}")

    v0694 = V0694_DOC.read_text(encoding="utf-8")
    require("narrow_public_safe_aaef_main_handback_material_preparation_planning_completed = true" in v0694, "v0.6.94 material preparation planning must be completed")
    require("narrow_public_safe_aaef_main_handback_material_candidate_may_be_considered = true" in v0694, "v0.6.94 must allow material candidate consideration")
    ...
    for category in EXPECTED_CATEGORIES:
        ...
        require("expected_failure_category" not in metadata, f"v0.6.96 must not add metadata-level failure category: {category}")
    ...
    for phrase in [
        ...
        require(phrase not in lower_doc, f"v0.6.96 document must not include affirmative overclaim phrase: {phrase}")

    print("v0.6.96 narrow public-safe AAEF main handback material preparation candidate review and close-readiness tests passed.")
    return 0
