...
DOC = ROOT / "docs/238-v06162-public-review-entry-point-polish-review-and-decision.md"
ADR = ROOT / "planning/decisions/ADR-0232-add-v06162-public-review-entry-point-polish-review-and-decision.md"
ISSUE = ROOT / "planning/issues/0231-add-v06162-public-review-entry-point-polish-review-and-decision.md"
...
REQUIRED_DOC_PHRASES = ['Status: decision', 'This is checkpoint 2 of 2 for a Medium-risk public-facing documentation work item.', 'This checkpoint reviews and accepts the Public Review Entry Point Polish candidate from v0.6.161.', 'The Public Review Entry Point Polish work item is closed.', 'Review conclusion', 'Candidate accepted', 'README public review entry point confirmed', 'External Review Package pointer confirmed', 'Review and orientation only language confirmed', 'Non-authorizing notice confirmed', 'Safe review order confirmed', 'Separate authorization statement confirmed', 'Claim boundaries confirmed', 'Reviewed README entry point', 'Review checklist', 'Work item closeout', 'What did not happen', 'Next direction', 'v0.6.163 Next Work Selection Using Risk-Tiered Granularity']
...
FLAGS = ['public_review_entry_point_polish_review_decision_completed = true', 'public_review_entry_point_polish_review_decision_is_medium_risk = true', 'public_review_entry_point_polish_review_decision_checkpoint_2_of_2 = true', 'public_review_entry_point_candidate_reviewed = true', 'public_review_entry_point_candidate_accepted = true', 'public_review_entry_point_polish_work_item_closed = true', 'readme_public_review_entry_point_reviewed = true', 'readme_public_review_entry_point_accepted = true', 'public_review_entry_points_to_external_review_package_confirmed = true', 'public_review_entry_intended_use_confirmed = true', 'public_review_entry_non_authorizing_notice_confirmed = true', 'public_review_entry_key_artifact_links_confirmed = true', 'public_review_entry_safe_review_order_confirmed = true', 'public_review_entry_separate_authorization_statement_confirmed = true', 'public_review_entry_claim_boundaries_confirmed = true', 'public_review_entry_reviewer_usefulness_confirmed = true', 'public_review_entry_point_created = true', 'public_review_entry_point_modified = true', 'readme_public_entry_modified = true', 'public_review_entry_review_decision_completed = true', 'review_decision_completed = true', 'external_review_package_modified = false', 'reviewer_walkthrough_modified = false', 'control_matrix_modified = false', 'safe_poc_boundary_template_modified = false', 'technical_due_diligence_summary_modified = false', 'enterprise_review_guide_modified = false', 'customer_poc_authorized = false', 'commercial_contract_created = false', 'validator_behavior_modified = false', 'metadata_level_expected_failure_category_added = false', 'json_schema_added = false', 'public_sample_changed = false', 'runtime_behavior_modified = false', 'runtime_execution_authorized = false', 'scanner_execution_authorized = false', 'docker_execution_authorized = false', 'credential_injection_permitted = false', 'customer_target_authorized = false', 'delivery_authorized = false', 'aaef_main_handback_sequence_remains_paused = true', 'aaef_main_handback_sequence_reopened = false', 'aaef_main_issue_opened = false', 'aaef_main_pull_request_opened = false', 'aaef_main_issue_submission_command_generated = false', 'aaef_main_issue_url_generated = false', 'certification_claim_occurs = false', 'legal_compliance_claim_occurs = false', 'audit_opinion_claim_occurs = false', 'production_readiness_claim_occurs = false', 'external_framework_equivalence_claim_occurs = false', 'diagnostic_completeness_claim_occurs = false', 'third_party_testing_authorization_claim_occurs = false', 'aaef_core_promotion = false', 'aaef_profile_promotion = false', 'aaef_practical_package_promotion = false', 'selected_next_direction = next_work_selection_using_risk_tiered_granularity', 'selected_next_checkpoint = v0.6.163 Next Work Selection Using Risk-Tiered Granularity']
...
def main() -> int:
    for path in [README, DOC, ADR, ISSUE, V06161_DOC, V06160_DOC, V06159_DOC, V06158_DOC, V06156_DOC, V06153_DOC, V06150_DOC, V06147_DOC, V06144_DOC, V06119_DOC]:
    ...
    for phrase in REQUIRED_README_PHRASES:
        require(phrase in readme_text, f"README missing reviewed public entry text: {phrase}")

    for phrase in REQUIRED_DOC_PHRASES + FLAGS:
        require(phrase in doc_text, f"v0.6.162 document missing required text: {phrase}")
    ...
    v06161 = V06161_DOC.read_text(encoding="utf-8")
    for phrase in [
        ...
        require(phrase in v06161, f"v0.6.161 candidate missing required phrase: {phrase}")
    ...
    for phrase in [
        ...
        require(phrase not in lower_doc, f"v0.6.162 document must not include affirmative forbidden phrase: {phrase}")

    print("v0.6.162 Public Review Entry Point Polish review and decision tests passed.")
    return 0
