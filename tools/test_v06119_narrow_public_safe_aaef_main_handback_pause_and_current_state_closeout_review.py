...
DOC = ROOT / "docs/195-v06119-narrow-public-safe-aaef-main-handback-pause-and-current-state-closeout-review.md"
V06118_DOC = ROOT / "docs/194-v06118-narrow-public-safe-aaef-main-handback-human-maintainer-final-submission-decision-or-pause.md"
V06117_DOC = ROOT / "docs/193-v06117-narrow-public-safe-aaef-main-handback-human-submission-checklist-review-close-readiness.md"
...
def main() -> int:
    for path in [DOC, V06118_DOC, V06117_DOC, V0655_DOC, INDEX, VALIDATOR]:
    ...
    for phrase in REQUIRED_PHRASES + REQUIRED_FLAGS:
        require(phrase in doc_text, f"v0.6.119 document missing required text: {phrase}")

    for category in EXPECTED_CATEGORIES:
        require(category in doc_text, f"v0.6.119 document missing retained category: {category}")
        ...
        require("expected_failure_category" not in metadata, f"v0.6.119 must not add metadata-level failure category: {category}")
    ...
    v06118 = V06118_DOC.read_text(encoding="utf-8")
    for phrase in [
        "narrow_public_safe_aaef_main_handback_human_maintainer_final_submission_decision_or_pause_completed = true",
        "pause_selected = true",
        "keep_internal_only_selected = true",
        "final_submission_selected = false",
        "human_only_issue_opening_path_selected = false",
        "human_maintainer_final_submission_authorized = false",
        "selected_next_step_after_human_maintainer_final_submission_decision_or_pause = narrow_public_safe_aaef_main_handback_pause_and_current_state_closeout_review",
        "narrow_public_safe_aaef_main_handback_pause_and_current_state_closeout_review_authorized_for_next_checkpoint = true",
        ...
        require(phrase in v06118, f"v0.6.118 baseline missing required phrase: {phrase}")

    v06117 = V06117_DOC.read_text(encoding="utf-8")
    require("human_submission_checklist_close_ready = true" in v06117, "v0.6.117 must mark checklist close-ready")
    ...
    for phrase in [
        ...
        "this closeout submits to aaef main",
        "this closeout opens an aaef main issue",
        "scanner execution is authorized by this closeout",
        "runtime execution is authorized by this closeout",
    ]:
        require(phrase not in lower_doc, f"v0.6.119 document must not include affirmative overclaim phrase: {phrase}")

    print("v0.6.119 narrow public-safe AAEF main handback pause and current-state closeout review tests passed.")
    return 0
