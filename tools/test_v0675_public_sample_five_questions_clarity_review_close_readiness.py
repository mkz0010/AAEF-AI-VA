...
DOC = ROOT / "docs/152-v0675-public-sample-five-questions-clarity-review-close-readiness.md"
...
def main() -> int:
    ...
    for flag in REQUIRED_DOC_FLAGS:
        require(flag in doc_text, f"v0.6.75 document missing close-readiness flag: {flag}")

    for area in [
        "Scope and non-goals",
        "Recommended reviewer reading path",
        "Five-questions clarity matrix",
        "Model request versus authority",
        "Static sample versus runtime execution",
        "Validator relationship",
        "Public sample can/cannot statements",
        "Deferred gaps",
    ]:
        require(area in doc_text, f"v0.6.75 document missing retained clarity area: {area}")
    ...
    for phrase in [
        ...
        require(phrase in doc_text, f"v0.6.75 document missing close-readiness check: {phrase}")
    ...
    for question in [
        ...
        require(question in doc_text, f"v0.6.75 document missing AAEF question: {question}")
    ...
    for phrase in [
        ...
        require(phrase in doc_text, f"v0.6.75 document missing validator boundary phrase: {phrase}")

    for category in EXPECTED_CATEGORIES:
        require(f"`{category}`" in doc_text, f"v0.6.75 document missing retained category: {category}")
        require((FIXTURE_ROOT / category).exists(), f"fixture directory missing: {category}")

    for field in REQUIRED_METADATA_FIELDS:
        require(field in doc_text, f"v0.6.75 document missing retained metadata field: {field}")

    for flag in REQUIRED_BOUNDARY_FLAGS:
        require(f"{flag} = false" in doc_text, f"v0.6.75 document missing retained false boundary flag: {flag}")
    ...
    for phrase in [
        ...
        require(phrase in doc_text, f"v0.6.75 document missing unresolved decision phrase: {phrase}")
    ...
    require(
        "public_sample_five_questions_clarity_candidate_close_readiness_may_be_considered = true" in v0674_text,
        "v0.6.74 must allow close-readiness consideration",
    )
    require(
        "public_sample_five_questions_clarity_current_sample_content_retained = true" in v0674_text,
        "v0.6.74 must retain current sample content",
    )
    require(
        "public_sample_five_questions_clarity_public_sample_changed = false" in v0674_text,
        "v0.6.74 must not change public sample",
    )
    require(
        "applied_evidence_sanitized_public_sample_refined = false" in v0674_text,
        "v0.6.74 must not refine sanitized public sample",
    )
    require("validator_behavior_modified = false" in v0674_text, "v0.6.74 must preserve validator behavior boundary")
    ...
    for category in EXPECTED_CATEGORIES:
        ...
        require("expected_failure_category" not in metadata, f"v0.6.75 must not add metadata-level failure category: {category}")
    ...
    for phrase in forbidden_affirmative_phrases:
        require(phrase not in lower_doc, f"v0.6.75 document must not include affirmative overclaim phrase: {phrase}")

    print("v0.6.75 public sample five-questions clarity review and close-readiness tests passed.")
    return 0
