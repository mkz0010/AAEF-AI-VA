...
def test_v06320_doc_tokens() -> None:
    assert_tokens(DOC, REQUIRED_DOC_TOKENS + REQUIRED_DECISION_TOKENS + REQUIRED_SHARED_TOKENS)

def test_v06320_adr_tokens() -> None:
    assert_tokens(ADR, [
        "ADR-0395",
        "Status: completed positioning review",
        "Record that the safe local-only runnable demo needs a public positioning candidate before any public wording integration.",
    ] + REQUIRED_DECISION_TOKENS + REQUIRED_SHARED_TOKENS)

def test_v06320_issue_tokens() -> None:
    assert_tokens(ISSUE, [
        "0395 - Add v0.6.320 Safe Local-Only Runnable Demo Public Positioning Review",
        "Status: completed by v0.6.320",
        ...
        "Proceed to v0.6.321 Safe Local-Only Runnable Demo Public Positioning Candidate",
    ])

def test_v06320_index_tokens() -> None:
...
def test_v06320_registered_in_run_all() -> None:
    assert_tokens(RUN_ALL, ["tools/test_v06320_safe_local_only_runnable_demo_public_positioning_review.py"])

def main() -> None:
    test_v06320_doc_tokens()
    test_v06320_adr_tokens()
    test_v06320_issue_tokens()
    test_v06320_index_tokens()
    test_v06320_registered_in_run_all()
    print("v0.6.320 safe local-only runnable demo public positioning review checks passed")
