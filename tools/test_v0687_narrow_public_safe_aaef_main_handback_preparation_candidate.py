...
DOC = ROOT / "docs/163-v0687-narrow-public-safe-aaef-main-handback-preparation-candidate.md"
...
def main() -> int:
    ...
    for flag in REQUIRED_DOC_FLAGS:
        require(flag in doc_text, f"v0.6.87 document missing candidate flag: {flag}")
    ...
    for phrase in [
        ...
        require(phrase in doc_text, f"v0.6.87 document missing candidate section or review gate: {phrase}")
    ...
    for phrase in [
        ...
        require(phrase in doc_text, f"v0.6.87 document missing retained lesson or boundary phrase: {phrase}")
    ...
    for phrase in [
        ...
        require(phrase in doc_text, f"v0.6.87 document missing unresolved decision phrase: {phrase}")

    for category in EXPECTED_CATEGORIES:
        require(f"`{category}`" in doc_text, f"v0.6.87 document missing retained category: {category}")
        require((FIXTURE_ROOT / category).exists(), f"fixture directory missing: {category}")

    for field in REQUIRED_METADATA_FIELDS:
        require(field in doc_text, f"v0.6.87 document missing retained metadata field: {field}")

    for flag in REQUIRED_BOUNDARY_FLAGS:
        require(f"{flag} = false" in doc_text, f"v0.6.87 document missing retained false boundary flag: {flag}")
    ...
    for phrase in [
        ...
        require(phrase in v0686, f"v0.6.86 baseline missing required phrase: {phrase}")

    v0685 = V0685_DOC.read_text(encoding="utf-8")
    require(
        "selected_next_step_after_handback_readiness_closeout = narrow_public_safe_aaef_main_handback_preparation_planning" in v0685,
        "v0.6.85 must select handback preparation planning",
    )
    ...
    for category in EXPECTED_CATEGORIES:
        ...
        require("expected_failure_category" not in metadata, f"v0.6.87 must not add metadata-level failure category: {category}")
    ...
    for phrase in [
        ...
        require(phrase not in lower_doc, f"v0.6.87 document must not include affirmative overclaim phrase: {phrase}")

    print("v0.6.87 narrow public-safe AAEF main handback preparation candidate tests passed.")
    return 0
