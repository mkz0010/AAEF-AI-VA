...
DOC = ROOT / "docs/192-v06116-narrow-public-safe-aaef-main-handback-human-submission-checklist-preparation.md"
...
def main() -> int:
    for path in [DOC, V06115_DOC, V06114_DOC, V0655_DOC, INDEX, VALIDATOR]:
    ...
    for phrase in REQUIRED_PHRASES + REQUIRED_FLAGS:
        require(phrase in doc_text, f"v0.6.116 document missing required text: {phrase}")

    for category in EXPECTED_CATEGORIES:
        require(category in doc_text, f"v0.6.116 document missing retained category: {category}")
        ...
        require("expected_failure_category" not in metadata, f"v0.6.116 must not add metadata-level failure category: {category}")
    ...
    for phrase in [
        ...
        require(phrase in v06115, f"v0.6.115 baseline missing required phrase: {phrase}")

    v06114 = V06114_DOC.read_text(encoding="utf-8")
    require("exact_issue_text_candidate_close_ready = true" in v06114, "v0.6.114 must mark issue candidate close-ready")
    ...
    for phrase in [
        ...
        require(phrase not in lower_doc, f"v0.6.116 document must not include affirmative overclaim phrase: {phrase}")

    print("v0.6.116 narrow public-safe AAEF main handback human submission checklist preparation tests passed.")
    return 0
