...
DOC = ROOT / "docs/171-v0695-narrow-public-safe-aaef-main-handback-material-preparation-candidate.md"
...
def main() -> int:
    ...
    require(V0693_DOC.exists(), "v0.6.93 material preparation decision document must exist")
    ...
    for flag in REQUIRED_DOC_FLAGS:
        require(flag in doc_text, f"v0.6.95 document missing candidate flag: {flag}")
    ...
    for phrase in [
        ...
        require(phrase in doc_text, f"v0.6.95 document missing two-layer or candidate phrase: {phrase}")
    ...
    for phrase in [
        ...
        require(phrase in doc_text, f"v0.6.95 document missing retained lesson or boundary phrase: {phrase}")
    ...
    for phrase in [
        ...
        require(phrase in doc_text, f"v0.6.95 document missing deferred external review item: {phrase}")

    for category in EXPECTED_CATEGORIES:
        require(category in doc_text, f"v0.6.95 document missing retained category: {category}")
        require((FIXTURE_ROOT / category).exists(), f"fixture directory missing: {category}")

    for field in REQUIRED_METADATA_FIELDS:
        require(field in doc_text, f"v0.6.95 document missing retained metadata field: {field}")

    for flag in REQUIRED_BOUNDARY_FLAGS:
        require(f"{flag} = false" in doc_text, f"v0.6.95 document missing retained false boundary flag: {flag}")
    ...
    for phrase in [
        ...
        require(phrase in v0694, f"v0.6.94 baseline missing required phrase: {phrase}")

    v0693 = V0693_DOC.read_text(encoding="utf-8")
    require(
        "selected_next_step_after_handback_drafting_closeout = narrow_public_safe_aaef_main_handback_material_preparation_planning" in v0693,
        "v0.6.93 must select material preparation planning",
    )
    ...
    for category in EXPECTED_CATEGORIES:
        ...
        require("expected_failure_category" not in metadata, f"v0.6.95 must not add metadata-level failure category: {category}")
    ...
    for phrase in [
        ...
        require(phrase not in lower_doc, f"v0.6.95 document must not include affirmative overclaim phrase: {phrase}")

    print("v0.6.95 narrow public-safe AAEF main handback material preparation candidate tests passed.")
    return 0
