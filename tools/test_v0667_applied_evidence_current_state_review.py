...
DOC = ROOT / "docs/144-v0667-applied-evidence-current-state-review.md"
...
def main() -> int:
    require(V0666_DOC.exists(), "v0.6.66 next-direction intake document must exist")
    ...
    for flag in REQUIRED_DOC_FLAGS:
        require(flag in doc_text, f"v0.6.67 document missing current-state flag: {flag}")
    ...
    for item in [
        ...
        require(item in doc_text, f"v0.6.67 document missing review surface or gap: {item}")
    ...
    for question in [
        ...
        require(question in doc_text, f"v0.6.67 document missing AAEF question: {question}")

    for category in EXPECTED_CATEGORIES:
        require(f"`{category}`" in doc_text, f"v0.6.67 document missing retained category: {category}")
        require((FIXTURE_ROOT / category).exists(), f"fixture directory missing: {category}")

    for field in REQUIRED_METADATA_FIELDS:
        require(field in doc_text, f"v0.6.67 document missing retained metadata field: {field}")

    for flag in REQUIRED_BOUNDARY_FLAGS:
        require(f"{flag} = false" in doc_text, f"v0.6.67 document missing retained false boundary flag: {flag}")
    ...
    for phrase in [
        ...
        require(phrase in doc_text, f"v0.6.67 document missing unresolved decision phrase: {phrase}")

    v0666_text = V0666_DOC.read_text(encoding="utf-8")
    require(
        "applied_evidence_next_direction_intake_completed = true" in v0666_text,
        "v0.6.66 intake must be completed",
    )
    require(
    ...
    require(
        "applied_evidence_implementation_started = false" in v0666_text,
        "v0.6.66 must not start Applied Evidence implementation",
    )
    require("validator_behavior_modified = false" in v0666_text, "v0.6.66 must preserve validator behavior boundary")
    ...
    for category in EXPECTED_CATEGORIES:
        ...
        require("expected_failure_category" not in metadata, f"v0.6.67 must not add metadata-level failure category: {category}")
    ...
    forbidden_affirmative_phrases = [
        ...
        "this current-state review provides certification",
        "this current-state review is legally sufficient",
        "this current-state review provides an audit opinion",
        "this project is authorized for customer delivery",
        "scanner execution is authorized by this current-state review",
        "runtime execution is authorized by this current-state review",
    ]
    for phrase in forbidden_affirmative_phrases:
        require(phrase not in lower_doc, f"v0.6.67 document must not include affirmative overclaim phrase: {phrase}")

    print("v0.6.67 applied evidence current-state review tests passed.")
    return 0
