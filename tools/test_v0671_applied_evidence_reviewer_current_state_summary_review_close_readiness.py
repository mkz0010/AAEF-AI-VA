...
DOC = ROOT / "docs/148-v0671-applied-evidence-reviewer-current-state-summary-review-close-readiness.md"
...
def main() -> int:
    ...
    for flag in REQUIRED_DOC_FLAGS:
        require(flag in doc_text, f"v0.6.71 document missing close-readiness flag: {flag}")

    for area in [
        "Scope and non-goals",
        "Current artifact map",
        "Public-safe sample baseline",
        "AAEF five-questions orientation",
        "Public validator relationship",
        "Non-execution and non-delivery boundary",
        "Gap summary",
        "Next checkpoint guidance",
    ]:
        require(area in doc_text, f"v0.6.71 document missing retained summary scope area: {area}")
    ...
    for phrase in [
        ...
        require(phrase in doc_text, f"v0.6.71 document missing close-readiness check: {phrase}")
    ...
    for question in [
        ...
        require(question in doc_text, f"v0.6.71 document missing AAEF question: {question}")
    ...
    for phrase in [
        ...
        require(phrase in doc_text, f"v0.6.71 document missing validator boundary phrase: {phrase}")

    for category in EXPECTED_CATEGORIES:
        require(f"`{category}`" in doc_text, f"v0.6.71 document missing retained category: {category}")
        require((FIXTURE_ROOT / category).exists(), f"fixture directory missing: {category}")

    for field in REQUIRED_METADATA_FIELDS:
        require(field in doc_text, f"v0.6.71 document missing retained metadata field: {field}")

    for flag in REQUIRED_BOUNDARY_FLAGS:
        require(f"{flag} = false" in doc_text, f"v0.6.71 document missing retained false boundary flag: {flag}")
    ...
    for phrase in [
        ...
        require(phrase in doc_text, f"v0.6.71 document missing deferred gap or unresolved decision phrase: {phrase}")
    ...
    require(
        "applied_evidence_reviewer_current_state_summary_candidate_close_readiness_may_be_considered = true" in v0670_text,
        "v0.6.70 must allow close-readiness consideration",
    )
    require(
        "applied_evidence_sanitized_public_sample_refined = false" in v0670_text,
        "v0.6.70 must not refine sanitized public sample",
    )
    require("validator_behavior_modified = false" in v0670_text, "v0.6.70 must preserve validator behavior boundary")
    ...
    for category in EXPECTED_CATEGORIES:
        ...
        require("expected_failure_category" not in metadata, f"v0.6.71 must not add metadata-level failure category: {category}")
    ...
    for phrase in forbidden_affirmative_phrases:
        require(phrase not in lower_doc, f"v0.6.71 document must not include affirmative overclaim phrase: {phrase}")

    print("v0.6.71 applied evidence reviewer current-state summary review and close-readiness tests passed.")
    return 0
