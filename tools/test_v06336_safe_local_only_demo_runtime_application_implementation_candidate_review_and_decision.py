...
def test_v06336_primary_files() -> None:
    assert_tokens(DOC, REQUIRED)
    assert_tokens(ADR, [
        "ADR-0411",
        "Status: accepted",
        "Accept the implementation candidate as a bounded planning candidate that remains not runtime-applied.",
    ])
    assert_tokens(ISSUE, [
        ...
        "Proceed to v0.6.337 Safe Local-Only Demo Runtime Application Implementation Closeout Review",
    ])

def test_v06336_index_files() -> None:
...
def test_v06336_registered_in_run_all() -> None:
    assert_tokens(RUN_ALL, ["tools/test_v06336_safe_local_only_demo_runtime_application_implementation_candidate_review_and_decision.py"])

def test_v06336_avoids_raw_forbidden_claim_phrases() -> None:
...
def main() -> None:
    test_v06336_primary_files()
    test_v06336_index_files()
    test_v06336_registered_in_run_all()
    test_v06336_avoids_raw_forbidden_claim_phrases()
    print("v0.6.336 safe local-only demo runtime application implementation candidate review and decision checks passed")
