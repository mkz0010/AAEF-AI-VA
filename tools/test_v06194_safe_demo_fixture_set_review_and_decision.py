...
DOC = ROOT / "docs/270-v06194-safe-demo-fixture-set-review-and-decision.md"
ADR = ROOT / "planning/decisions/ADR-0264-add-v06194-safe-demo-fixture-set-review-and-decision.md"
ISSUE = ROOT / "planning/issues/0263-add-v06194-safe-demo-fixture-set-review-and-decision.md"
...
FORBIDDEN_DOC_PHRASES = ['public demo is created in this checkpoint', 'executable demo is created in this checkpoint', 'runtime work is started in this checkpoint', 'scanner work is started in this checkpoint', 'real scanner execution is selected now', 'runtime execution is selected now', 'customer poc intake is selected now', 'customer poc is authorized', 'commercial contract is created', 'commercial license terms are created', 'paid engagement is approved', 'runtime execution is authorized', 'scanner execution is authorized', 'docker execution is authorized', 'credential injection is permitted', 'customer target is authorized', 'delivery is authorized', 'authorized to test third-party systems', 'permission to test third-party systems is granted', 'aaef main contact publication completed', 'aaef main repository modified = true', 'aaef main handback sequence reopened = true', 'is certified as', 'certified framework for', 'legally compliant with', 'legal compliance achieved', 'audit opinion provided', 'deployment approved', 'is production ready', 'production-ready scanner', 'complete vulnerability scanner', 'equivalent to nist', 'equivalent to iso', 'ai can safely run tools without gates']
...
def main() -> int:
    for path in [FIXTURE_DIR, REQUEST, DECISION, RESULT, TRACE, WALKTHROUGH, DOC, ADR, ISSUE, V06193, V06192, V06191, V06190, V06187, V06184, V06181, V06172, V06120, V06119]:
    ...
    for phrase in REQUIRED + FLAGS:
        require(phrase in doc_text, f"v0.6.194 document missing required text: {phrase}")
    ...
    print("v0.6.194 Safe Demo Fixture Set review and decision tests passed.")
    return 0
