...
DOC = ROOT / "docs/150-v0673-public-sample-five-questions-clarity-planning.md"
...
def main() -> int:
    ...
    for flag in REQUIRED_DOC_FLAGS:
        require(flag in doc_text, f"v0.6.73 document missing planning flag: {flag}")
    ...
    for question in [
        ...
        require(question in doc_text, f"v0.6.73 document missing AAEF question: {question}")
    ...
    for phrase in [
        ...
        "Do not claim runtime execution or live authorization.",
    ]:
        require(phrase in doc_text, f"v0.6.73 document missing planned clarity phrase: {phrase}")
    ...
    for phrase in [
        ...
        require(phrase in doc_text, f"v0.6.73 document missing future candidate acceptance check: {phrase}")
    ...
    for phrase in [
        ...
        require(phrase in doc_text, f"v0.6.73 document missing validator boundary phrase: {phrase}")

    for category in EXPECTED_CATEGORIES:
        require(f"`{category}`" in doc_text, f"v0.6.73 document missing retained category: {category}")
        require((FIXTURE_ROOT / category).exists(), f"fixture directory missing: {category}")

    for field in REQUIRED_METADATA_FIELDS:
        require(field in doc_text, f"v0.6.73 document missing retained metadata field: {field}")

    for flag in REQUIRED_BOUNDARY_FLAGS:
        require(f"{flag} = false" in doc_text, f"v0.6.73 document missing retained false boundary flag: {flag}")
    ...
    for phrase in [
        ...
        require(phrase in doc_text, f"v0.6.73 document missing unresolved decision phrase: {phrase}")
    ...
    require(
        "public_sample_five_questions_clarity_started = false" in v0672_text,
        "v0.6.72 must not start public sample five-questions clarity",
    )
    require(
        "public_sample_five_questions_clarity_public_sample_changed = false" in v0672_text,
        "v0.6.72 must not change public sample",
    )
    require("validator_behavior_modified = false" in v0672_text, "v0.6.72 must preserve validator behavior boundary")
    ...
    require(
        ...
        "v0.6.71 summary close-readiness must be completed",
    )
    ...
    for category in EXPECTED_CATEGORIES:
        ...
        require("expected_failure_category" not in metadata, f"v0.6.73 must not add metadata-level failure category: {category}")
    ...
    forbidden_affirmative_phrases = [
        ...
        "this clarity planning provides certification",
        "this clarity planning is legally sufficient",
        "this clarity planning provides an audit opinion",
        "this project is authorized for customer delivery",
        "scanner execution is authorized by this clarity planning",
        "runtime execution is authorized by this clarity planning",
    ]
    for phrase in forbidden_affirmative_phrases:
        require(phrase not in lower_doc, f"v0.6.73 document must not include affirmative overclaim phrase: {phrase}")

    print("v0.6.73 public sample five-questions clarity planning tests passed.")
    return 0
