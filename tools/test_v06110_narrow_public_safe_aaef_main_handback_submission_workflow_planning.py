...
DOC = ROOT / "docs/186-v06110-narrow-public-safe-aaef-main-handback-submission-workflow-planning.md"
...
def main() -> int:
    for path in [DOC, V06109_DOC, V06108_DOC, V0655_DOC, INDEX, VALIDATOR]:
    ...
    for phrase in REQUIRED_PHRASES + REQUIRED_FLAGS:
        require(phrase in doc_text, f"v0.6.110 document missing required text: {phrase}")

    for category in EXPECTED_CATEGORIES:
        require(category in doc_text, f"v0.6.110 document missing retained category: {category}")
        ...
        require("expected_failure_category" not in metadata, f"v0.6.110 must not add metadata-level failure category: {category}")
    ...
    for phrase in [
        ...
        require(phrase in v06109, f"v0.6.109 baseline missing required phrase: {phrase}")

    v06108 = V06108_DOC.read_text(encoding="utf-8")
    require(
        "narrow_public_safe_aaef_main_handback_submittable_text_preparation_candidate_close_ready = true" in v06108,
        "v0.6.108 must close the internal candidate",
    )
    ...
    for phrase in [
        ...
        require(phrase not in lower_doc, f"v0.6.110 document must not include affirmative overclaim phrase: {phrase}")

    print("v0.6.110 narrow public-safe AAEF main handback submission workflow planning tests passed.")
    return 0
