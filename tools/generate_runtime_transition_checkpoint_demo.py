...
def build_demo_preflight_pair() -> tuple[dict, dict]:
    ...
    preflight_gate = evaluate_preflight_validation_gate(preflight)
    return preflight, preflight_gate
...
def main() -> int:
    ...
    out_dir = ROOT / "private-not-in-git" / "runtime-transition-checkpoint" / "demo"
    preflight_path = out_dir / "preflight-validation.generated.json"
    preflight_gate_path = out_dir / "preflight-validation-gate-result.generated.json"
    ...
    write_json(preflight_gate_path, preflight_gate)
    write_json(checkpoint_path, checkpoint)
    write_text(checkpoint_md_path, format_runtime_transition_checkpoint_markdown(checkpoint))

    print(f"runtime transition checkpoint:    {checkpoint_path}")
    print(f"runtime transition checkpoint md: {checkpoint_md_path}")
    print(f"checkpoint status:                {checkpoint['checkpoint_status']}")
    print(f"ready for preflight implementation: {checkpoint['readiness_summary']['ready_for_preflight_implementation']}")
    print(f"ready for runtime execution:        {checkpoint['readiness_summary']['ready_for_runtime_execution']}")
    return 0
