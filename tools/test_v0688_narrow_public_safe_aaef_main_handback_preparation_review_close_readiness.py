...
DOC = ROOT / "docs/164-v0688-narrow-public-safe-aaef-main-handback-preparation-review-close-readiness.md"
V0687_DOC = ROOT / "docs/163-v0687-narrow-public-safe-aaef-main-handback-preparation-candidate.md"
V0686_DOC = ROOT / "docs/162-v0686-narrow-public-safe-aaef-main-handback-preparation-planning.md"
...
REQUIRED_DOC_FLAGS = [
    "narrow_public_safe_aaef_main_handback_preparation_review_completed = true",
    "narrow_public_safe_aaef_main_handback_preparation_close_ready = true",
    "close_narrow_public_safe_aaef_main_handback_preparation_candidate = true",
    "retain_v0687_narrow_public_safe_aaef_main_handback_preparation_candidate = true",
    "narrow_public_safe_aaef_main_handback_preparation_candidate_retained = true",
    "narrow_public_safe_aaef_main_handback_preparation_candidate_reviewed = true",
    "narrow_public_safe_aaef_main_handback_preparation_candidate_scope_preserved = true",
    "narrow_public_safe_aaef_main_handback_preparation_candidate_revision_required = false",
    "narrow_public_safe_aaef_main_handback_preparation_candidate_replaced = false",
    "narrow_public_safe_aaef_main_handback_preparation_candidate_uses_v0687_candidate = true",
    ...
    "narrow_public_safe_aaef_main_handback_preparation_candidate_reviewer_aid_retained = true",
    "narrow_public_safe_aaef_main_handback_preparation_candidate_close_ready = true",
...
def main() -> int:
    require(V0687_DOC.exists(), "v0.6.87 handback preparation candidate document must exist")
    require(V0686_DOC.exists(), "v0.6.86 handback preparation planning document must exist")
    ...
    for flag in REQUIRED_DOC_FLAGS:
        require(flag in doc_text, f"v0.6.88 document missing close-readiness flag: {flag}")
    ...
    for phrase in [
        ...
        "Internal-only candidate gate",
        "AAEF main submission gate",
        "Handback material gate",
        "Sensitive-detail gate",
        "Private-material gate",
        "Scanner/credential/customer gate",
        "Delivery gate",
        "Claim boundary gate",
        "Promotion gate",
        ...
        "Candidate does not write AAEF main handback text",
        ...
        require(phrase in doc_text, f"v0.6.88 document missing close-readiness section or check: {phrase}")
    ...
    for phrase in [
        ...
        require(phrase in doc_text, f"v0.6.88 document missing retained lesson or boundary phrase: {phrase}")
    ...
    for phrase in [
        ...
        require(phrase in doc_text, f"v0.6.88 document missing unresolved decision phrase: {phrase}")

    for category in EXPECTED_CATEGORIES:
        require(f"`{category}`" in doc_text, f"v0.6.88 document missing retained category: {category}")
        require((FIXTURE_ROOT / category).exists(), f"fixture directory missing: {category}")

    for field in REQUIRED_METADATA_FIELDS:
        require(field in doc_text, f"v0.6.88 document missing retained metadata field: {field}")

    for flag in REQUIRED_BOUNDARY_FLAGS:
        require(f"{flag} = false" in doc_text, f"v0.6.88 document missing retained false boundary flag: {flag}")

    v0687 = V0687_DOC.read_text(encoding="utf-8")
    for phrase in [
        ...
        require(phrase in v0687, f"v0.6.87 baseline missing required phrase: {phrase}")

    v0686 = V0686_DOC.read_text(encoding="utf-8")
    require("narrow_public_safe_aaef_main_handback_preparation_planning_completed = true" in v0686, "v0.6.86 planning must be completed")
    require("narrow_public_safe_aaef_main_handback_preparation_candidate_may_be_considered = true" in v0686, "v0.6.86 must allow candidate consideration")
    ...
    for category in EXPECTED_CATEGORIES:
        ...
        require("expected_failure_category" not in metadata, f"v0.6.88 must not add metadata-level failure category: {category}")
    ...
    for phrase in [
        ...
        require(phrase not in lower_doc, f"v0.6.88 document must not include affirmative overclaim phrase: {phrase}")

    print("v0.6.88 narrow public-safe AAEF main handback preparation review and close-readiness tests passed.")
    return 0
