...
DOC = ROOT / "docs/130-v0653-public-validator-failure-category-mapping-candidate.md"
...
def main() -> int:
    ...
    doc_text = require_text(
        ...
        [
            "Status: candidate",
            "documentation-only candidate mapping",
    ...
    for flag in REQUIRED_DOC_FLAGS:
        require(flag in doc_text, f"v0.6.53 document missing candidate flag: {flag}")

    for category, failure_category in CATEGORY_TO_FAILURE.items():
        require(f"`{category}`" in doc_text, f"v0.6.53 document missing negative category: {category}")
        require(f"`{failure_category}`" in doc_text, f"v0.6.53 document missing failure category: {failure_category}")
        require((FIXTURE_ROOT / category).exists(), f"fixture directory missing: {category}")

    for phrase in [
        "Keep documentation-only mapping",
        "Add metadata-level `expected_failure_category`",
        "Add validator JSON output failure category names",
        "Add JSON Schema for mapping",
    ]:
        require(phrase in doc_text, f"v0.6.53 document missing future gate phrase: {phrase}")

    v0652_text = V0652_DOC.read_text(encoding="utf-8")
    require(
        "failure_category_mapping_candidate_may_be_considered = true" in v0652_text,
        "v0.6.52 must allow a mapping candidate",
    )
    require(
        "failure_category_mapping_implemented = false" in v0652_text,
        "v0.6.52 must not already implement mapping",
    )
    require(
        "failure_category_mapping_validator_output_added = false" in v0652_text,
        "v0.6.52 must not add validator output",
    )
    require("validator_behavior_modified = false" in v0652_text, "v0.6.52 must preserve validator behavior boundary")

    v0651_text = V0651_DOC.read_text(encoding="utf-8")
    require(
        "metadata_contract_track_close_ready = true" in v0651_text,
        "v0.6.51 metadata contract track must be close-ready",
    )
    require(
        "close_public_validator_negative_fixture_metadata_contract_track = true" in v0651_text,
        "v0.6.51 must close metadata contract track",
    )
    ...
    for category in CATEGORY_TO_FAILURE:
        ...
        require("expected_failure_category" not in metadata, f"v0.6.53 must not add metadata-level failure category: {category}")
    ...
    for phrase in forbidden_affirmative_phrases:
        require(phrase not in lower_doc, f"v0.6.53 document must not include affirmative overclaim phrase: {phrase}")

    print("v0.6.53 public validator failure category mapping candidate tests passed.")
    return 0
