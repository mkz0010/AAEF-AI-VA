...
DOC = ROOT / "docs/181-v06105-applied-evidence-handback-submittable-text-or-pause-decision.md"
...
def main() -> int:
    for path in [DOC, V06104_DOC, V06103_DOC, V0655_DOC, INDEX, VALIDATOR]:
    ...
    for phrase in REQUIRED_PHRASES + REQUIRED_FLAGS:
        require(phrase in doc_text, f"v0.6.105 document missing required text: {phrase}")

    for category in EXPECTED_CATEGORIES:
        require(category in doc_text, f"v0.6.105 document missing retained category: {category}")
        ...
        require("expected_failure_category" not in metadata, f"v0.6.105 must not add metadata-level failure category: {category}")
    ...
    for phrase in [
        "narrow_public_safe_aaef_main_handback_final_text_preparation_candidate_review_completed = true",
        "narrow_public_safe_aaef_main_handback_final_text_preparation_candidate_close_ready = true",
        "aaef_main_handback_text_submittable_preparation_or_pause_decision_may_be_considered = true",
        ...
        require(phrase in v06104, f"v0.6.104 baseline missing required phrase: {phrase}")

    v06103 = V06103_DOC.read_text(encoding="utf-8")
    require("narrow_public_safe_aaef_main_handback_final_text_candidate_generated = true" in v06103, "v0.6.103 final-text candidate must exist")
    require("narrow_public_safe_aaef_main_handback_final_text_candidate_submittable = false" in v06103, "v0.6.103 final-text candidate must remain non-submittable")
    ...
    for phrase in [
        ...
        require(phrase not in lower_doc, f"v0.6.105 document must not include affirmative overclaim phrase: {phrase}")

    print("v0.6.105 applied evidence handback submittable text or pause decision tests passed.")
    return 0
