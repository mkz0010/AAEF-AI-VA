...
DOC = ROOT / "docs/236-v06160-next-work-selection-using-risk-tiered-granularity.md"
ADR = ROOT / "planning/decisions/ADR-0230-add-v06160-next-work-selection-using-risk-tiered-granularity.md"
ISSUE = ROOT / "planning/issues/0229-add-v06160-next-work-selection-using-risk-tiered-granularity.md"
...
REQUIRED = ['Status: decision', 'This checkpoint applies the v0.6.120 risk-tiered checkpoint granularity policy.', 'This checkpoint is intentionally completed as one Low-risk checkpoint.', 'The selected next work item is Public Review Entry Point Polish.', 'The selected next work item risk tier is Medium risk.', 'The selected checkpoint count is 2 checkpoints.', 'The next checkpoint is v0.6.161 Public Review Entry Point Polish Candidate.', 'This checkpoint does not modify the README public entry point.', 'This checkpoint does not modify runtime behavior.', 'This checkpoint does not reopen the AAEF main handback sequence.', 'No AAEF main issue is opened.', 'No issue creation command is generated.', 'No issue URL is created.', 'Risk-tiered selection record', 'Candidate work items reviewed', 'Selected work item', 'Why this is Medium risk', 'Why not High or Critical risk', 'Why not Low risk', 'Planned checkpoint split', 'Expected Public Review Entry Point Polish scope', 'Claim boundaries for the selected work', 'What did not happen', 'Next direction']
FLAGS = ['next_work_selection_using_risk_tiered_granularity_v06160_completed = true', 'next_work_selection_v06160_is_documentation_only = true', 'next_work_selection_v06160_uses_v06120_granularity_policy = true', 'next_work_selection_v06160_uses_v06159_completed_work_item = true', 'v06160_completed_as_low_risk_one_checkpoint = true', 'selected_next_work_item = public_review_entry_point_polish', 'selected_next_work_item_risk_tier = medium', 'selected_next_work_item_checkpoint_count = 2', 'selected_next_work_item_first_checkpoint = candidate_implementation', 'selected_next_work_item_second_checkpoint = review_and_decision', 'selected_next_checkpoint = v0.6.161 Public Review Entry Point Polish Candidate', 'public_review_entry_point_polish_selected = true', 'external_review_package_integration_work_item_closed = true', 'reviewer_walkthrough_work_item_closed = true', 'control_matrix_work_item_closed = true', 'safe_poc_boundary_template_work_item_closed = true', 'technical_due_diligence_summary_work_item_closed = true', 'enterprise_review_guide_work_item_closed = true', 'aaef_main_handback_sequence_remains_paused = true', 'aaef_main_handback_sequence_reopened = false', 'aaef_main_issue_opened = false', 'aaef_main_pull_request_opened = false', 'aaef_main_issue_submission_command_generated = false', 'aaef_main_issue_url_generated = false', 'public_review_entry_point_created = false', 'public_review_entry_point_modified = false', 'readme_public_entry_modified = false', 'external_review_package_modified = false', 'reviewer_walkthrough_modified = false', 'control_matrix_modified = false', 'safe_poc_boundary_template_modified = false', 'technical_due_diligence_summary_modified = false', 'enterprise_review_guide_modified = false', 'customer_poc_authorized = false', 'commercial_contract_created = false', 'validator_behavior_modified = false', 'metadata_level_expected_failure_category_added = false', 'json_schema_added = false', 'public_sample_changed = false', 'runtime_behavior_modified = false', 'runtime_execution_authorized = false', 'scanner_execution_authorized = false', 'docker_execution_authorized = false', 'credential_injection_permitted = false', 'customer_target_authorized = false', 'delivery_authorized = false', 'certification_claim_occurs = false', 'legal_compliance_claim_occurs = false', 'audit_opinion_claim_occurs = false', 'production_readiness_claim_occurs = false', 'external_framework_equivalence_claim_occurs = false', 'diagnostic_completeness_claim_occurs = false', 'third_party_testing_authorization_claim_occurs = false', 'aaef_core_promotion = false', 'aaef_profile_promotion = false', 'aaef_practical_package_promotion = false']
...
def main() -> int:
    for path in [DOC, ADR, ISSUE, V06159_DOC, V06158_DOC, V06156_DOC, V06153_DOC, V06150_DOC, V06147_DOC, V06144_DOC, V06120_DOC, V06119_DOC]:
        require(path.exists(), f"missing required file: {path.relative_to(ROOT)}")

    doc_text = DOC.read_text(encoding="utf-8")
    for phrase in REQUIRED + FLAGS:
        require(phrase in doc_text, f"v0.6.160 document missing required text: {phrase}")

    v06159 = V06159_DOC.read_text(encoding="utf-8")
    for phrase in [
        ...
        require(phrase in v06159, f"v0.6.159 baseline missing required phrase: {phrase}")
    ...
    for phrase in [
        "this checkpoint creates the public review entry point",
        "this checkpoint modifies the readme public entry point",
        ...
        require(phrase not in lower_doc, f"v0.6.160 document must not include affirmative forbidden phrase: {phrase}")

    print("v0.6.160 next work selection using risk-tiered granularity tests passed.")
    return 0
