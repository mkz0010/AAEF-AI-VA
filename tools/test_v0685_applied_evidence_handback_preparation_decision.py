...
DOC = ROOT / "docs/161-v0685-applied-evidence-handback-preparation-decision.md"
V0684_DOC = ROOT / "docs/160-v0684-evidence-interface-handback-readiness-review-close-readiness.md"
...
def main() -> int:
    require(V0684_DOC.exists(), "v0.6.84 handback readiness closeout document must exist")
    ...
    for flag in REQUIRED_DOC_FLAGS:
        require(flag in doc_text, f"v0.6.85 document missing decision flag: {flag}")

    for option in [
        "Narrow public-safe AAEF main handback preparation planning",
        "Prepare AAEF main handback text now",
        ...
        require(option in doc_text, f"v0.6.85 document missing option review row: {option}")
    ...
    for phrase in [
        ...
        require(phrase in doc_text, f"v0.6.85 document missing next-step boundary phrase: {phrase}")
    ...
    for lesson in [
        ...
        require(lesson in doc_text, f"v0.6.85 document missing retained eligible lesson: {lesson}")
    ...
    for question in [
        ...
        require(question in doc_text, f"v0.6.85 document missing AAEF question: {question}")

    for category in EXPECTED_CATEGORIES:
        require(f"`{category}`" in doc_text, f"v0.6.85 document missing retained category: {category}")
        require((FIXTURE_ROOT / category).exists(), f"fixture directory missing: {category}")

    for field in REQUIRED_METADATA_FIELDS:
        require(field in doc_text, f"v0.6.85 document missing retained metadata field: {field}")

    for flag in REQUIRED_BOUNDARY_FLAGS:
        require(f"{flag} = false" in doc_text, f"v0.6.85 document missing retained false boundary flag: {flag}")
    ...
    for phrase in [
        ...
        require(phrase in doc_text, f"v0.6.85 document missing unresolved decision phrase: {phrase}")

    v0684 = V0684_DOC.read_text(encoding="utf-8")
    for phrase in [
        ...
        require(phrase in v0684, f"v0.6.84 baseline missing required phrase: {phrase}")

    v0683 = V0683_DOC.read_text(encoding="utf-8")
    require("evidence_interface_handback_readiness_candidate_lesson_family = public_safe_evidence_interface_boundary_lessons" in v0683, "v0.6.83 must identify candidate lesson family")
    ...
    for category in EXPECTED_CATEGORIES:
        ...
        require("expected_failure_category" not in metadata, f"v0.6.85 must not add metadata-level failure category: {category}")
    ...
    for phrase in [
        ...
        require(phrase not in lower_doc, f"v0.6.85 document must not include affirmative overclaim phrase: {phrase}")

    print("v0.6.85 applied evidence handback preparation decision tests passed.")
    return 0
