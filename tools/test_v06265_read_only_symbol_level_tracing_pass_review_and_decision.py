...
DOC = ROOT / "docs/340-v06265-read-only-symbol-level-tracing-pass-review-and-decision.md"
ADR = ROOT / "planning/decisions/ADR-0340-add-v06265-read-only-symbol-level-tracing-pass-review-and-decision.md"
ISSUE = ROOT / "planning/issues/0340-add-v06265-read-only-symbol-level-tracing-pass-review-and-decision.md"
...
def test_v06265_doc_tokens() -> None:
    assert_tokens(DOC, REQUIRED_DOC_TOKENS + REQUIRED_DECISION_TOKENS + REQUIRED_COMPONENT_TOKENS + REQUIRED_SCOPE_TOKENS)


def test_v06265_adr_tokens() -> None:
    assert_tokens(
        ADR,
        [
            "ADR-0340",
            "Status: accepted",
            "Accept the v0.6.264 read-only symbol-level tracing pass as static inspection records",
            "recommended_next_work_item = narrower_manual_trace_review",
            "Read-only symbol-level tracing results are static inspection records.",
            "Observed source symbols are not proof of pre-dispatch enforcement.",
            "Observed call-path status records are not full gateway integration proof.",
...
def test_v06265_issue_tokens() -> None:
    assert_tokens(
        ISSUE,
        [
            ...
            "recommended_next_work_item = narrower_manual_trace_review",
            "accepted_defect_records_created = false",
            "Proceed to v0.6.266 Next Work Selection Using Risk-Tiered Granularity",
        ],
    )


def test_v06265_index_tokens() -> None:
    assert_tokens(
        README,
        [
            "v0.6.265 Read-Only Symbol-Level Tracing Pass Review and Decision",
    ...
    assert_tokens(
        ...
        [
            "v0.6.265 - Read-Only Symbol-Level Tracing Pass Review and Decision",
            "Accepted the v0.6.264 Read-Only Symbol-Level Tracing Pass as static inspection records",
            "read_only_symbol_level_tracing_pass_review_completed = true",
            "read_only_symbol_level_tracing_pass_accepted = true",
            "read_only_symbol_level_tracing_pass_id = read_only_symbol_level_tracing_pass_v06264",
            "trace_record_schema",
...
def test_v06265_registered_in_run_all() -> None:
    assert_tokens(RUN_ALL, ["tools/test_v06265_read_only_symbol_level_tracing_pass_review_and_decision.py"])


def main() -> None:
    test_v06265_doc_tokens()
    test_v06265_adr_tokens()
    test_v06265_issue_tokens()
    test_v06265_index_tokens()
    test_v06265_registered_in_run_all()
    print("v0.6.265 read-only symbol-level tracing pass review and decision checks passed")
