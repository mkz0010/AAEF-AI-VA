...
DOC = ROOT / "docs/151-v0674-public-sample-five-questions-clarity-candidate.md"
V0673_DOC = ROOT / "docs/150-v0673-public-sample-five-questions-clarity-planning.md"
V0672_DOC = ROOT / "docs/149-v0672-applied-evidence-next-gap-selection-review.md"
...
def main() -> int:
    require(V0673_DOC.exists(), "v0.6.73 clarity planning document must exist")
    require(V0672_DOC.exists(), "v0.6.72 next-gap selection document must exist")
    ...
    for flag in REQUIRED_DOC_FLAGS:
        require(flag in doc_text, f"v0.6.74 document missing candidate flag: {flag}")
    ...
    for section in [
        ...
        require(section in doc_text, f"v0.6.74 document missing clarity section: {section}")
    ...
    for question in [
        ...
        require(question in doc_text, f"v0.6.74 document missing AAEF question: {question}")

    for phrase in [
        "Separate model output from authority.",
        "Locate the authority-relevant decision point.",
        "Do not invent actors, tools, executions, or results that are not represented in the current public sample.",
        "Model output is not authority.",
        "Do not claim runtime execution, live authorization, scanner authorization, or customer-target authorization.",
        "Do not claim live proof, audit sufficiency, legal sufficiency, compliance, certification, or diagnostic completeness.",
        ...
        require(phrase in doc_text, f"v0.6.74 document missing clarity phrase: {phrase}")
    ...
    for phrase in [
        ...
        require(phrase in doc_text, f"v0.6.74 document missing validator boundary phrase: {phrase}")

    for phrase in [
        "Candidate preserves the current public sample content",
        "Candidate maps each AAEF five question to current sample reading guidance",
        "Candidate distinguishes model request from authority",
        "Candidate distinguishes static evidence-interface demonstration from live proof",
        "Candidate explains where validator checks help and where they stop",
        ...
        require(phrase in doc_text, f"v0.6.74 document missing candidate acceptance check: {phrase}")

    for category in EXPECTED_CATEGORIES:
        require(f"`{category}`" in doc_text, f"v0.6.74 document missing retained category: {category}")
        require((FIXTURE_ROOT / category).exists(), f"fixture directory missing: {category}")

    for field in REQUIRED_METADATA_FIELDS:
        require(field in doc_text, f"v0.6.74 document missing retained metadata field: {field}")

    for flag in REQUIRED_BOUNDARY_FLAGS:
        require(f"{flag} = false" in doc_text, f"v0.6.74 document missing retained false boundary flag: {flag}")
    ...
    for phrase in [
        ...
        require(phrase in doc_text, f"v0.6.74 document missing unresolved decision phrase: {phrase}")

    v0673_text = V0673_DOC.read_text(encoding="utf-8")
    require(
        "public_sample_five_questions_clarity_planning_completed = true" in v0673_text,
        "v0.6.73 clarity planning must be completed",
    )
    require(
        "public_sample_five_questions_clarity_candidate_may_be_considered = true" in v0673_text,
        "v0.6.73 must allow clarity candidate consideration",
    )
    require(
        "public_sample_five_questions_clarity_public_sample_changed = false" in v0673_text,
        "v0.6.73 must not change public sample",
    )
    require(
        "applied_evidence_sanitized_public_sample_refined = false" in v0673_text,
        "v0.6.73 must not refine sanitized public sample",
    )
    require("validator_behavior_modified = false" in v0673_text, "v0.6.73 must preserve validator behavior boundary")

    v0672_text = V0672_DOC.read_text(encoding="utf-8")
    require(
        "applied_evidence_next_gap_selected = public_sample_five_questions_clarity" in v0672_text,
        "v0.6.72 must select public sample five-questions clarity",
    )
    ...
    for category in EXPECTED_CATEGORIES:
        ...
        require("expected_failure_category" not in metadata, f"v0.6.74 must not add metadata-level failure category: {category}")
    ...
    forbidden_affirmative_phrases = [
        ...
        "this clarity candidate provides certification",
        "this clarity candidate is legally sufficient",
        "this clarity candidate provides an audit opinion",
        "this project is authorized for customer delivery",
        "scanner execution is authorized by this clarity candidate",
        "runtime execution is authorized by this clarity candidate",
    ]
    for phrase in forbidden_affirmative_phrases:
        require(phrase not in lower_doc, f"v0.6.74 document must not include affirmative overclaim phrase: {phrase}")

    print("v0.6.74 public sample five-questions clarity candidate tests passed.")
    return 0
