...
from evidence_chain import build_evidence_chain, format_evidence_chain_markdown
...
def main() -> int:
    ...
    out_dir = ROOT / "private-not-in-git" / "evidence-chains" / "demo"
    chain_path = out_dir / "evidence-chain.generated.json"
    markdown_path = out_dir / "evidence-chain.generated.md"

    write_json(chain_path, chain)
    write_text(markdown_path, format_evidence_chain_markdown(chain))

    print(f"evidence chain json: {chain_path}")
    print(f"evidence chain md:   {markdown_path}")
    print(f"evidence chain status: {chain['chain_status']}")
    return 0
