from __future__ import annotations

from pathlib import Path


ROOT = Path(__file__).resolve().parents[1]

DOC = ROOT / "docs/292-v06216-public-exposure-hygiene-plan-candidate.md"
ADR = ROOT / "planning/decisions/ADR-0286-add-v06216-public-exposure-hygiene-plan-candidate.md"
ISSUE = ROOT / "planning/issues/0285-add-v06216-public-exposure-hygiene-plan-candidate.md"
README = ROOT / "README.md"
CHANGELOG = ROOT / "CHANGELOG.md"
ROADMAP = ROOT / "ROADMAP.md"
RUN_ALL = ROOT / "tools/run_all_local_checks.py"


REQUIRED_DOC_TOKENS = [
    "Public Exposure Hygiene Plan Candidate",
    "Status: Candidate only",
    "candidate only",
    "not accepted",
    "not applied",
    "README front-page value proposition and reader path",
    "Public contact route hygiene",
    "Pricing and commercial draft exposure hygiene",
    "Business plan exposure hygiene",
    "Current/latest version clarity",
    "External-facing candidate and draft label hygiene",
    "Public documentation curation",
    "Demo and motion-evidence positioning",
    "Candidate sequencing",
    "Candidate definition of done for future cleanup work",
    "Treatment of existing public history",
    "runtime demo remains necessary but deferred",
    "Publication remains deferred",
]


REQUIRED_BOUNDARY_TOKENS = [
    "v0.6.216 does not",
    "remove public contact routes",
    "move pricing materials",
    "move business-plan materials",
    "rewrite the README front page",
    "delete historical docs",
    "create a mock execution recording",
    "create a runtime demo",
    "implement Gateway core safety controls",
    "change Tool Gateway behavior",
    "rename execution statuses",
]


REQUIRED_REPOSITORY_TOKENS = [
    "v0.6.216",
    "Public Exposure Hygiene Plan Candidate",
    "candidate only",
    "not applied",
    "v0.6.217 Public Exposure Hygiene Plan Review and Decision",
    "no cleanup is applied in v0.6.216",
    "runtime demo remains necessary but deferred",
    "publication remains deferred",
]


FORBIDDEN_TOKENS = [
    "public_contact_route_removed = true",
    "pricing_materials_moved = true",
    "business_plan_moved = true",
    "readme_front_page_rewritten = true",
    "historical_docs_deleted = true",
    "mock_execution_recording_created = true",
    "gateway_core_safety_controls_implemented = true",
    "tool_gateway_behavior_changed = true",
    "adapter_behavior_changed = true",
    "execution_status_renamed = true",
    "runtime_demo_ready = true",
    "scanner_readiness_claim = true",
    "external_poc_readiness_claim = true",
    "publication_approval = true",
    "public_announcement = approved",
    "social_post_publication = approved",
    "production_readiness_claim = true",
    "certification_claim = true",
    "legal_compliance_claim = true",
    "audit_opinion_claim = true",
    "diagnostic_completeness_claim = true",
    "external_framework_equivalence_claim = true",
]


def read(path: Path) -> str:
    assert path.exists(), f"missing expected file: {path.relative_to(ROOT)}"
    return path.read_text(encoding="utf-8")


def assert_contains_all(path: Path, tokens: list[str]) -> None:
    text = read(path)
    for token in tokens:
        assert token in text, f"{path.relative_to(ROOT)} missing token: {token}"


def test_v06216_files_exist_and_record_plan_candidate() -> None:
    assert_contains_all(DOC, REQUIRED_DOC_TOKENS)
    assert_contains_all(DOC, REQUIRED_BOUNDARY_TOKENS)
    assert_contains_all(ADR, ["Status: Candidate recorded", "candidate plan is not accepted", "v0.6.217 must review"])
    assert_contains_all(ISSUE, ["Public Exposure Hygiene Plan Candidate", "no cleanup is applied in v0.6.216", "v0.6.217 must review"])


def test_v06216_repository_surfaces_plan_candidate() -> None:
    for path in [README, CHANGELOG, ROADMAP]:
        assert_contains_all(path, REQUIRED_REPOSITORY_TOKENS)


def test_v06216_does_not_apply_cleanup_gateway_runtime_publication_or_assurance_changes() -> None:
    for path in [DOC, ADR, ISSUE, README, CHANGELOG, ROADMAP]:
        text = read(path)
        for token in FORBIDDEN_TOKENS:
            assert token not in text, f"{path.relative_to(ROOT)} contains forbidden token: {token}"


def test_v06216_run_all_registers_local_test() -> None:
    run_all = read(RUN_ALL)
    assert "tools/test_v06216_public_exposure_hygiene_plan_candidate.py" in run_all


if __name__ == "__main__":
    test_v06216_files_exist_and_record_plan_candidate()
    test_v06216_repository_surfaces_plan_candidate()
    test_v06216_does_not_apply_cleanup_gateway_runtime_publication_or_assurance_changes()
    test_v06216_run_all_registers_local_test()
    print("v0.6.216 Public Exposure Hygiene Plan Candidate checks passed")
