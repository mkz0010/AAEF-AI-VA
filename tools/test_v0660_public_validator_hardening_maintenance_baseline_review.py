...
DOC = ROOT / "docs/137-v0660-public-validator-hardening-maintenance-baseline-review.md"
...
def main() -> int:
    ...
    for flag in REQUIRED_DOC_FLAGS:
        require(flag in doc_text, f"v0.6.60 document missing maintenance baseline flag: {flag}")

    for category in EXPECTED_CATEGORIES:
        require(f"`{category}`" in doc_text, f"v0.6.60 document missing retained category: {category}")
    ...
    for phrase in [
        ...
        "next-step clarity",
        "Add a public validator negative fixture index summary",
        "Add a reviewer navigation note for v0.6.44-v0.6.60",
        "Group negative fixture tests in `run_all_local_checks.py` comments",
        "Improve fixture metadata explanations without field changes",
        "Improve mapping documentation layout",
        "Clarify hardening scope non-implementation wording",
        "Reduce duplicate prose across review documents",
        ...
        require(phrase in doc_text, f"v0.6.60 document missing maintenance or deferred path phrase: {phrase}")
    ...
    require(
        "public_validator_behavior_hardening_implementation_readiness_deferred = true" in v0659_text,
        "v0.6.59 must defer behavior hardening implementation readiness",
    )
    require(
        "validator_behavior_hardening_implementation_may_be_considered = false" in v0659_text,
        "v0.6.59 must not approve hardening implementation",
    )
    require("validator_behavior_modified = false" in v0659_text, "v0.6.59 must preserve validator behavior boundary")
    ...
    for category in EXPECTED_CATEGORIES:
        ...
        require("expected_failure_category" not in metadata, f"v0.6.60 must not add metadata-level failure category: {category}")
    ...
    forbidden_affirmative_phrases = [
        ...
        "this maintenance baseline provides certification",
        "this maintenance baseline is legally sufficient",
        "this maintenance baseline provides an audit opinion",
        "this project is authorized for customer delivery",
        "scanner execution is authorized by this maintenance baseline",
        "runtime execution is authorized by this maintenance baseline",
    ]
    for phrase in forbidden_affirmative_phrases:
        require(phrase not in lower_doc, f"v0.6.60 document must not include affirmative overclaim phrase: {phrase}")

    print("v0.6.60 public validator hardening maintenance baseline review tests passed.")
    return 0
