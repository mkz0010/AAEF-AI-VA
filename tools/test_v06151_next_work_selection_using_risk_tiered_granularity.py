...
DOC = ROOT / "docs/227-v06151-next-work-selection-using-risk-tiered-granularity.md"
ADR = ROOT / "planning/decisions/ADR-0221-add-v06151-next-work-selection-using-risk-tiered-granularity.md"
ISSUE = ROOT / "planning/issues/0220-add-v06151-next-work-selection-using-risk-tiered-granularity.md"
...
REQUIRED = ['Status: decision', 'This checkpoint applies the v0.6.120 risk-tiered checkpoint granularity policy.', 'This checkpoint is intentionally completed as one Low-risk checkpoint.', 'The selected next work item is Control Matrix.', 'The selected next work item risk tier is Medium risk.', 'The selected checkpoint count is 2 checkpoints.', 'The next checkpoint is v0.6.152 Control Matrix Candidate.', 'This checkpoint does not create the Control Matrix.', 'This checkpoint does not modify runtime behavior.', 'This checkpoint does not reopen the AAEF main handback sequence.', 'No AAEF main issue is opened.', 'No issue creation command is generated.', 'No issue URL is created.', 'Risk-tiered selection record', 'Candidate work items reviewed', 'Selected work item', 'Why this is Medium risk', 'Why not High or Critical risk', 'Why not Low risk', 'Planned checkpoint split', 'Expected Control Matrix scope', 'Claim boundaries for the selected work', 'What did not happen', 'Next direction']
FLAGS = ['next_work_selection_using_risk_tiered_granularity_v06151_completed = true', 'next_work_selection_v06151_is_documentation_only = true', 'next_work_selection_v06151_uses_v06120_granularity_policy = true', 'next_work_selection_v06151_uses_v06150_completed_work_item = true', 'v06151_completed_as_low_risk_one_checkpoint = true', 'selected_next_work_item = control_matrix', 'selected_next_work_item_risk_tier = medium', 'selected_next_work_item_checkpoint_count = 2', 'selected_next_work_item_first_checkpoint = candidate_implementation', 'selected_next_work_item_second_checkpoint = review_and_decision', 'selected_next_checkpoint = v0.6.152 Control Matrix Candidate', 'control_matrix_selected = true', 'safe_poc_boundary_template_work_item_closed = true', 'technical_due_diligence_summary_work_item_closed = true', 'enterprise_review_guide_work_item_closed = true', 'mock_dry_run_completed_status_terminology_cleanup_work_item_closed = true', 'external_discovery_fail_closed_behavior_work_item_closed = true', 'request_decision_constraint_diff_enforcement_work_item_closed = true', 'authorization_expiry_current_time_check_work_item_closed = true', 'reviewer_walkthrough_selected = false', 'aaef_main_handback_sequence_remains_paused = true', 'aaef_main_handback_sequence_reopened = false', 'aaef_main_issue_opened = false', 'aaef_main_pull_request_opened = false', 'aaef_main_issue_submission_command_generated = false', 'aaef_main_issue_url_generated = false', 'control_matrix_created = false', 'control_matrix_behavior_modified = false', 'safe_poc_boundary_template_created = false', 'safe_poc_boundary_template_modified = false', 'technical_due_diligence_summary_created = false', 'technical_due_diligence_summary_modified = false', 'enterprise_review_guide_created = false', 'enterprise_review_guide_modified = false', 'reviewer_walkthrough_created = false', 'customer_poc_authorized = false', 'commercial_contract_created = false', 'mock_completed_status_renamed = false', 'mock_dry_run_status_behavior_modified = false', 'external_discovery_fail_closed_added = false', 'constraint_diff_enforcement_added = false', 'authorization_expiry_now_check_added = false', 'validator_behavior_modified = false', 'metadata_level_expected_failure_category_added = false', 'json_schema_added = false', 'public_sample_changed = false', 'runtime_behavior_modified = false', 'runtime_execution_authorized = false', 'scanner_execution_authorized = false', 'docker_execution_authorized = false', 'credential_injection_permitted = false', 'customer_target_authorized = false', 'delivery_authorized = false', 'certification_claim_occurs = false', 'legal_compliance_claim_occurs = false', 'audit_opinion_claim_occurs = false', 'production_readiness_claim_occurs = false', 'external_framework_equivalence_claim_occurs = false', 'diagnostic_completeness_claim_occurs = false', 'third_party_testing_authorization_claim_occurs = false', 'aaef_core_promotion = false', 'aaef_profile_promotion = false', 'aaef_practical_package_promotion = false']
...
def main() -> int:
    for path in [DOC, ADR, ISSUE, V06150_DOC, V06149_DOC, V06147_DOC, V06144_DOC, V06141_DOC, V06138_DOC, V06134_DOC, V06130_DOC, V06120_DOC, V06119_DOC]:
        require(path.exists(), f"missing required file: {path.relative_to(ROOT)}")

    doc_text = DOC.read_text(encoding="utf-8")
    for phrase in REQUIRED + FLAGS:
        require(phrase in doc_text, f"v0.6.151 document missing required text: {phrase}")
    ...
    for phrase in [
        ...
        require(phrase in v06150, f"v0.6.150 baseline missing required phrase: {phrase}")
    ...
    require("safe_poc_boundary_template_created = true" in v06149, "v0.6.149 Safe PoC Boundary Template creation must remain recorded")

    v06147 = V06147_DOC.read_text(encoding="utf-8")
    require("technical_due_diligence_summary_work_item_closed = true" in v06147, "v0.6.147 Technical Due Diligence Summary work item must be closed")
    ...
    for phrase in [
        "this checkpoint creates the control matrix",
        ...
        require(phrase not in lower_doc, f"v0.6.151 document must not include affirmative forbidden phrase: {phrase}")

    print("v0.6.151 next work selection using risk-tiered granularity tests passed.")
    return 0
