...
DOC = ROOT / "docs/146-v0669-applied-evidence-reviewer-current-state-summary-planning.md"
...
def main() -> int:
    ...
    for flag in REQUIRED_DOC_FLAGS:
        require(flag in doc_text, f"v0.6.69 document missing planning flag: {flag}")
    ...
    for audience in [
        ...
        require(audience in doc_text, f"v0.6.69 document missing intended audience: {audience}")
    ...
    for section in [
        ...
        require(section in doc_text, f"v0.6.69 document missing planned section: {section}")
    ...
    for item in [
        ...
        require(item in doc_text, f"v0.6.69 document missing planned content boundary item: {item}")
    ...
    for question in [
        ...
        require(question in doc_text, f"v0.6.69 document missing AAEF question: {question}")
    ...
    for phrase in [
        ...
        require(phrase in doc_text, f"v0.6.69 document missing validator non-check phrase: {phrase}")

    for category in EXPECTED_CATEGORIES:
        require(f"`{category}`" in doc_text, f"v0.6.69 document missing retained category: {category}")
        require((FIXTURE_ROOT / category).exists(), f"fixture directory missing: {category}")

    for field in REQUIRED_METADATA_FIELDS:
        require(field in doc_text, f"v0.6.69 document missing retained metadata field: {field}")

    for flag in REQUIRED_BOUNDARY_FLAGS:
        require(f"{flag} = false" in doc_text, f"v0.6.69 document missing retained false boundary flag: {flag}")
    ...
    for phrase in [
        ...
        require(phrase in doc_text, f"v0.6.69 document missing unresolved decision phrase: {phrase}")

    v0668_text = V0668_DOC.read_text(encoding="utf-8")
    require(
        "applied_evidence_gap_prioritization_review_completed = true" in v0668_text,
        "v0.6.68 gap prioritization must be completed",
    )
    require(
    ...
    require(
        "applied_evidence_current_state_summary_generated = false" in v0668_text,
        "v0.6.68 must not generate current-state summary",
    )
    require("validator_behavior_modified = false" in v0668_text, "v0.6.68 must preserve validator behavior boundary")
    ...
    for category in EXPECTED_CATEGORIES:
        ...
        require("expected_failure_category" not in metadata, f"v0.6.69 must not add metadata-level failure category: {category}")
    ...
    forbidden_affirmative_phrases = [
        ...
        "this summary planning provides certification",
        "this summary planning is legally sufficient",
        "this summary planning provides an audit opinion",
        "this project is authorized for customer delivery",
        "scanner execution is authorized by this summary planning",
        "runtime execution is authorized by this summary planning",
    ]
    for phrase in forbidden_affirmative_phrases:
        require(phrase not in lower_doc, f"v0.6.69 document must not include affirmative overclaim phrase: {phrase}")

    print("v0.6.69 applied evidence reviewer current-state summary planning tests passed.")
    return 0
