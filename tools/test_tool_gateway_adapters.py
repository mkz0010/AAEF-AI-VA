...
def main() -> int:
    ...
    assert_true(command_plan["target_binding"]["target_id"] == "webapp-demo-001", "ZAP plan should include target binding")
    assert_true(command_plan["target_binding"]["raw_destination_included"] is False, "ZAP plan must not expose raw destination")
    assert_true(command_plan["target_binding"]["network_destination_ref"].startswith("scope-registry://"), "ZAP plan must use target ref")
