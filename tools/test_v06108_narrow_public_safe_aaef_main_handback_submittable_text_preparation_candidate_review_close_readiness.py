...
DOC = ROOT / "docs/184-v06108-narrow-public-safe-aaef-main-handback-submittable-text-preparation-candidate-review-close-readiness.md"
...
def main() -> int:
    for path in [DOC, V06107_DOC, V06106_DOC, V0655_DOC, INDEX, VALIDATOR]:
    ...
    for phrase in REQUIRED_PHRASES + REQUIRED_FLAGS:
        require(phrase in doc_text, f"v0.6.108 document missing required text: {phrase}")

    for category in EXPECTED_CATEGORIES:
        require(category in doc_text, f"v0.6.108 document missing retained category: {category}")
        ...
        require("expected_failure_category" not in metadata, f"v0.6.108 must not add metadata-level failure category: {category}")
    ...
    for phrase in [
        ...
        require(phrase in v06107, f"v0.6.107 baseline missing required phrase: {phrase}")

    v06106 = V06106_DOC.read_text(encoding="utf-8")
    require(
        "aaef_main_handback_submittable_text_candidate_authorized_for_next_checkpoint = true" in v06106,
        "v0.6.106 must authorize the submittable text candidate checkpoint",
    )
    ...
    for phrase in [
        ...
        require(phrase not in lower_doc, f"v0.6.108 document must not include affirmative overclaim phrase: {phrase}")

    print("v0.6.108 narrow public-safe AAEF main handback submittable text preparation candidate review and close-readiness tests passed.")
    return 0
