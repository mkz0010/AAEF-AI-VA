...
DEFAULT_OUTPUT_DIR = Path("private-not-in-git/prototype-runs")

SCHEMA_BY_GENERATED_NAME = {
    "tool-execution-result.generated.json": Path("schemas/tool-execution-result.schema.json"),
    "evidence-record.generated.json": Path("schemas/evidence-record.schema.json"),
}
...
def main() -> int:
    ...
    if not DEFAULT_OUTPUT_DIR.exists():
        errors.append(
            f"Generated output directory does not exist: {DEFAULT_OUTPUT_DIR}. "
            "Run: py tools/run_tool_gateway_example.py all"
        )
    else:
        generated_files = sorted(DEFAULT_OUTPUT_DIR.glob("*/*.generated.json"))
        if not generated_files:
            errors.append(
                f"No generated output files found under {DEFAULT_OUTPUT_DIR}. "
                "Run: py tools/run_tool_gateway_example.py all"
            )

        for path in generated_files:
            schema_path = SCHEMA_BY_GENERATED_NAME.get(path.name)
            if schema_path is None:
                errors.append(f"Unexpected generated output filename: {path}")
                continue
            if not schema_path.exists():
                errors.append(f"Missing schema file: {schema_path}")
                continue

            errors.extend(validate_against_schema(path, schema_path))

            data = load_json(path)
            if isinstance(data, dict):
                if "_adapter_output" in data:
                    errors.append(f"{path}: _adapter_output must not appear in generated public output")
                if data.get("secret_exposed_to_ai") is not False:
                    errors.append(f"{path}: secret_exposed_to_ai must be false")
