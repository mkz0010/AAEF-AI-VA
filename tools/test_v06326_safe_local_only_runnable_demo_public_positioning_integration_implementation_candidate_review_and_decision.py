...
def test_v06326_doc_tokens() -> None:
    assert_tokens(DOC, REQUIRED_DOC_TOKENS + REQUIRED_DECISION_TOKENS + REQUIRED_SHARED_TOKENS)

def test_v06326_adr_tokens() -> None:
    assert_tokens(ADR, [
        "ADR-0401",
        "Status: accepted",
        "Accept the v0.6.325 implementation candidate as bounded README status and boundary wording.",
    ] + REQUIRED_DECISION_TOKENS + REQUIRED_SHARED_TOKENS)

def test_v06326_issue_tokens() -> None:
    assert_tokens(ISSUE, [
        "0401 - Add v0.6.326 Safe Local-Only Runnable Demo Public Positioning Integration Implementation Candidate Review and Decision",
        "Status: completed by v0.6.326",
        ...
        "Proceed to v0.6.327 Safe Local-Only Runnable Demo Public Positioning Integration Closeout Review",
    ])

def test_v06326_index_tokens() -> None:
...
def test_v06326_registered_in_run_all() -> None:
    assert_tokens(RUN_ALL, ["tools/test_v06326_safe_local_only_runnable_demo_public_positioning_integration_implementation_candidate_review_and_decision.py"])

def test_v06326_avoids_raw_forbidden_public_positioning_claims() -> None:
...
def main() -> None:
    test_v06326_doc_tokens()
    test_v06326_adr_tokens()
    test_v06326_issue_tokens()
    test_v06326_index_tokens()
    test_v06326_registered_in_run_all()
    test_v06326_avoids_raw_forbidden_public_positioning_claims()
    print("v0.6.326 safe local-only runnable demo public positioning integration implementation candidate review and decision checks passed")
