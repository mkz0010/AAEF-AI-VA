...
DOC = ROOT / "docs/165-v0689-applied-evidence-handback-drafting-decision.md"
...
def main() -> int:
    ...
    for flag in REQUIRED_DOC_FLAGS:
        require(flag in doc_text, f"v0.6.89 document missing decision flag: {flag}")

    for option in [
        "Narrow public-safe AAEF main handback drafting planning",
        "Prepare AAEF main handback draft now",
        ...
        require(option in doc_text, f"v0.6.89 document missing option review row: {option}")
    ...
    for phrase in [
        ...
        require(phrase in doc_text, f"v0.6.89 document missing next-step boundary phrase: {phrase}")
    ...
    for phrase in [
        ...
        require(phrase in doc_text, f"v0.6.89 document missing retained lesson or boundary phrase: {phrase}")
    ...
    for phrase in [
        ...
        require(phrase in doc_text, f"v0.6.89 document missing unresolved decision phrase: {phrase}")

    for category in EXPECTED_CATEGORIES:
        require(f"`{category}`" in doc_text, f"v0.6.89 document missing retained category: {category}")
        require((FIXTURE_ROOT / category).exists(), f"fixture directory missing: {category}")

    for field in REQUIRED_METADATA_FIELDS:
        require(field in doc_text, f"v0.6.89 document missing retained metadata field: {field}")

    for flag in REQUIRED_BOUNDARY_FLAGS:
        require(f"{flag} = false" in doc_text, f"v0.6.89 document missing retained false boundary flag: {flag}")
    ...
    for phrase in [
        ...
        require(phrase in v0688, f"v0.6.88 baseline missing required phrase: {phrase}")

    v0687 = V0687_DOC.read_text(encoding="utf-8")
    require(
        "narrow_public_safe_aaef_main_handback_preparation_candidate_added = true" in v0687,
        "v0.6.87 preparation candidate must exist",
    )
    ...
    for category in EXPECTED_CATEGORIES:
        ...
        require("expected_failure_category" not in metadata, f"v0.6.89 must not add metadata-level failure category: {category}")
    ...
    for phrase in [
        ...
        require(phrase not in lower_doc, f"v0.6.89 document must not include affirmative overclaim phrase: {phrase}")

    print("v0.6.89 applied evidence handback drafting decision tests passed.")
    return 0
