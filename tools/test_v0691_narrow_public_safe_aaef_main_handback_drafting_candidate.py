...
DOC = ROOT / "docs/167-v0691-narrow-public-safe-aaef-main-handback-drafting-candidate.md"
...
REQUIRED_DOC_FLAGS = [
    ...
    "narrow_public_safe_aaef_main_handback_drafting_candidate_is_documentation_only = true",
    "narrow_public_safe_aaef_main_handback_drafting_candidate_uses_v0690_planning = true",
    "narrow_public_safe_aaef_main_handback_drafting_candidate_uses_v0689_decision = true",
    "narrow_public_safe_aaef_main_handback_drafting_candidate_generated = true",
    ...
    "narrow_public_safe_aaef_main_handback_drafting_candidate_reviewer_aid_generated = true",
...
def main() -> int:
    require(V0690_DOC.exists(), "v0.6.90 handback drafting planning document must exist")
    ...
    doc_text = require_text(
        ...
        [
            "Status: candidate",
            "It is a drafting candidate checkpoint only.",
            ...
            "It does not create a final handback draft.",
    ...
    for flag in REQUIRED_DOC_FLAGS:
        require(flag in doc_text, f"v0.6.91 document missing candidate flag: {flag}")
    ...
    for phrase in [
        ...
        require(phrase in doc_text, f"v0.6.91 document missing candidate section or review gate: {phrase}")
    ...
    for phrase in [
        ...
        require(phrase in doc_text, f"v0.6.91 document missing wording/audience phrase: {phrase}")
    ...
    for phrase in [
        ...
        require(phrase in doc_text, f"v0.6.91 document missing retained lesson or boundary phrase: {phrase}")

    for phrase in [
        "whether this handback drafting candidate is close-ready",
        ...
        require(phrase in doc_text, f"v0.6.91 document missing unresolved decision phrase: {phrase}")

    for category in EXPECTED_CATEGORIES:
        require(f"`{category}`" in doc_text, f"v0.6.91 document missing retained category: {category}")
        require((FIXTURE_ROOT / category).exists(), f"fixture directory missing: {category}")

    for field in REQUIRED_METADATA_FIELDS:
        require(field in doc_text, f"v0.6.91 document missing retained metadata field: {field}")

    for flag in REQUIRED_BOUNDARY_FLAGS:
        require(f"{flag} = false" in doc_text, f"v0.6.91 document missing retained false boundary flag: {flag}")
    ...
    for phrase in [
        ...
        require(phrase in v0690, f"v0.6.90 baseline missing required phrase: {phrase}")

    v0689 = V0689_DOC.read_text(encoding="utf-8")
    require(
        "selected_next_step_after_handback_preparation_closeout = narrow_public_safe_aaef_main_handback_drafting_planning" in v0689,
        "v0.6.89 must select handback drafting planning",
    )
    ...
    for category in EXPECTED_CATEGORIES:
        ...
        require("expected_failure_category" not in metadata, f"v0.6.91 must not add metadata-level failure category: {category}")
    ...
    for phrase in [
        ...
        "this candidate prepares aaef main handback material",
        ...
        require(phrase not in lower_doc, f"v0.6.91 document must not include affirmative overclaim phrase: {phrase}")

    print("v0.6.91 narrow public-safe AAEF main handback drafting candidate tests passed.")
    return 0
