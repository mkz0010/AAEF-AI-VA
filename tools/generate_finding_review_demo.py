...
from finding_review import build_finding_review_record, evaluate_finding_review_gate, format_finding_review_markdown
...
def main() -> int:
    review_decision = sys.argv[1] if len(sys.argv) > 1 else "needs_more_info"
    ...
    out_dir = ROOT / "private-not-in-git" / "finding-reviews" / "demo"
    ...
    gate_path = out_dir / "finding-review-gate-result.generated.json"
    md_path = out_dir / "finding-review.generated.md"

    write_json(raw_path, raw)
    sanitized = sanitize_raw_artifact(
        raw,
        raw_artifact_ref="private-not-in-git/finding-reviews/demo/raw-artifact.demo.json",
        sanitized_artifact_ref="private-not-in-git/finding-reviews/demo/sanitized-artifact.generated.json",
    )
    ...
    gate = evaluate_finding_review_gate(candidate, review)

    write_json(review_path, review)
    write_json(gate_path, gate)
    write_text(md_path, format_finding_review_markdown(review, gate))

    print(f"finding candidate: {candidate_path}")
    print(f"finding review:    {review_path}")
    print(f"finding gate:      {gate_path}")
    print(f"finding review md: {md_path}")
    print(f"review gate status: {gate['gate_status']}")
    return 0
