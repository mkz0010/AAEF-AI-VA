...
DOC = ROOT / "docs/174-v0698-narrow-public-safe-aaef-main-handback-text-drafting-planning.md"
...
def main() -> int:
    for path in [DOC, V0697_DOC, V0696_DOC, V0655_DOC, INDEX, VALIDATOR]:
    ...
    required_phrases = [
        ...
        "AAEF-AI-VA shows, as an Applied Implementation example",
    ...
    for phrase in required_phrases + REQUIRED_FLAGS:
        require(phrase in doc_text, f"v0.6.98 document missing required text: {phrase}")

    for category in EXPECTED_CATEGORIES:
        require(category in doc_text, f"v0.6.98 document missing retained category: {category}")
        ...
        require("expected_failure_category" not in metadata, f"v0.6.98 must not add metadata-level failure category: {category}")
    ...
    for phrase in [
        ...
        require(phrase in v0697, f"v0.6.97 baseline missing required phrase: {phrase}")

    v0696 = V0696_DOC.read_text(encoding="utf-8")
    require("narrow_public_safe_aaef_main_handback_material_preparation_candidate_close_ready = true" in v0696, "v0.6.96 material preparation candidate must be close-ready")
    ...
    for phrase in [
        ...
        "this planning writes final aaef main handback text",
        ...
        require(phrase not in lower_doc, f"v0.6.98 document must not include affirmative overclaim phrase: {phrase}")

    print("v0.6.98 narrow public-safe AAEF main handback text drafting planning tests passed.")
    return 0
