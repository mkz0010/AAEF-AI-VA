...
DOC = ROOT / "docs/229-v06153-control-matrix-review-and-decision.md"
ADR = ROOT / "planning/decisions/ADR-0223-add-v06153-control-matrix-review-and-decision.md"
ISSUE = ROOT / "planning/issues/0222-add-v06153-control-matrix-review-and-decision.md"
...
REQUIRED_DOC_PHRASES = ['Status: decision', 'This is checkpoint 2 of 2 for a Medium-risk reviewer-facing documentation work item.', 'This checkpoint reviews and accepts the Control Matrix candidate from v0.6.152.', 'The Control Matrix work item is closed.', 'Review conclusion', 'Candidate accepted', 'Reader and purpose confirmed', 'Non-authorizing notice confirmed', 'Matrix row design confirmed', 'Review questions confirmed', 'Control boundaries confirmed', 'Expected evidence confirmed', 'Related artifacts confirmed', 'Explicit non-goals confirmed', 'Reviewer notes confirmed', 'Matrix rows confirmed', 'Interpretation limits confirmed', 'Claim boundaries confirmed', 'Reviewed matrix sections', 'Review checklist', 'Work item closeout', 'What did not happen', 'Next direction', 'v0.6.154 Next Work Selection Using Risk-Tiered Granularity']
...
REQUIRED_ROW_IDS = ['CM-01', 'CM-02', 'CM-03', 'CM-04', 'CM-05', 'CM-06', 'CM-07', 'CM-08', 'CM-09', 'CM-10', 'CM-11', 'CM-12', 'CM-13', 'CM-14']
REQUIRED_COLUMNS = ['Review question', 'Control boundary', 'Expected evidence', 'Related artifacts', 'Explicit non-goal', 'Reviewer note']
FORBIDDEN_AFFIRMATIVE_PHRASES = ['customer poc is authorized', 'commercial contract is created', 'runtime execution is authorized', 'scanner execution is authorized', 'docker execution is authorized', 'credential injection is permitted', 'customer target is authorized', 'delivery is authorized', 'authorized to test third-party systems', 'authorized for third-party testing by this matrix', 'permission to test third-party systems is granted', 'is certified as', 'certified framework for', 'legally compliant with', 'legal compliance achieved', 'audit opinion provided', 'audit sufficiency achieved', 'deployment approved', 'is production ready', 'production-ready scanner', 'complete vulnerability scanner', 'complete vulnerability assessment capability is provided', 'equivalent to nist', 'equivalent to iso', 'safe to run without gates', 'ai can safely run tools without gates', 'aaef core promotion = true', 'aaef profile promotion = true', 'aaef practical package promotion = true']
FLAGS = ['control_matrix_review_decision_completed = true', 'control_matrix_review_decision_is_medium_risk = true', 'control_matrix_review_decision_checkpoint_2_of_2 = true', 'control_matrix_candidate_reviewed = true', 'control_matrix_candidate_accepted = true', 'control_matrix_work_item_closed = true', 'control_matrix_file_reviewed = true', 'control_matrix_reader_confirmed = true', 'control_matrix_non_authorizing_notice_confirmed = true', 'control_matrix_row_design_confirmed = true', 'control_matrix_review_questions_confirmed = true', 'control_matrix_control_boundaries_confirmed = true', 'control_matrix_expected_evidence_confirmed = true', 'control_matrix_related_artifacts_confirmed = true', 'control_matrix_explicit_non_goals_confirmed = true', 'control_matrix_reviewer_notes_confirmed = true', 'control_matrix_ai_request_boundary_confirmed = true', 'control_matrix_gate_decision_boundary_confirmed = true', 'control_matrix_authorization_expiry_boundary_confirmed = true', 'control_matrix_constraint_drift_boundary_confirmed = true', 'control_matrix_external_discovery_boundary_confirmed = true', 'control_matrix_mock_dry_run_status_boundary_confirmed = true', 'control_matrix_non_execution_evidence_boundary_confirmed = true', 'control_matrix_human_approval_boundary_confirmed = true', 'control_matrix_credential_data_boundary_confirmed = true', 'control_matrix_public_private_artifact_boundary_confirmed = true', 'control_matrix_report_delivery_boundary_confirmed = true', 'control_matrix_poc_non_authorization_boundary_confirmed = true', 'control_matrix_commercial_license_boundary_confirmed = true', 'control_matrix_conservative_claim_boundaries_confirmed = true', 'control_matrix_interpretation_limits_confirmed = true', 'control_matrix_reviewer_usefulness_confirmed = true', 'control_matrix_created = true', 'control_matrix_review_decision_completed = true', 'review_decision_completed = true', 'customer_poc_authorized = false', 'commercial_contract_created = false', 'validator_behavior_modified = false', 'metadata_level_expected_failure_category_added = false', 'json_schema_added = false', 'public_sample_changed = false', 'runtime_behavior_modified = false', 'runtime_execution_authorized = false', 'scanner_execution_authorized = false', 'docker_execution_authorized = false', 'credential_injection_permitted = false', 'customer_target_authorized = false', 'delivery_authorized = false', 'aaef_main_handback_sequence_remains_paused = true', 'aaef_main_handback_sequence_reopened = false', 'aaef_main_issue_opened = false', 'aaef_main_pull_request_opened = false', 'aaef_main_issue_submission_command_generated = false', 'aaef_main_issue_url_generated = false', 'safe_poc_boundary_template_modified = false', 'technical_due_diligence_summary_modified = false', 'enterprise_review_guide_modified = false', 'reviewer_walkthrough_created = false', 'mock_completed_status_renamed = false', 'mock_dry_run_status_behavior_modified = false', 'external_discovery_fail_closed_added = false', 'constraint_diff_enforcement_added = false', 'authorization_expiry_now_check_added = false', 'certification_claim_occurs = false', 'legal_compliance_claim_occurs = false', 'audit_opinion_claim_occurs = false', 'production_readiness_claim_occurs = false', 'external_framework_equivalence_claim_occurs = false', 'diagnostic_completeness_claim_occurs = false', 'third_party_testing_authorization_claim_occurs = false', 'aaef_core_promotion = false', 'aaef_profile_promotion = false', 'aaef_practical_package_promotion = false', 'selected_next_direction = next_work_selection_using_risk_tiered_granularity', 'selected_next_checkpoint = v0.6.154 Next Work Selection Using Risk-Tiered Granularity']
...
def main() -> int:
    for path in [MATRIX, DOC, ADR, ISSUE, V06152_DOC, V06151_DOC, V06150_DOC, V06149_DOC, V06147_DOC, V06144_DOC, V06141_DOC, V06138_DOC, V06134_DOC, V06130_DOC, V06119_DOC]:
    ...
    for phrase in REQUIRED_DOC_PHRASES + FLAGS:
        require(phrase in doc_text, f"v0.6.153 document missing required text: {phrase}")

    for row_id in REQUIRED_ROW_IDS:
        require(row_id in matrix_text, f"Control Matrix missing reviewed row id: {row_id}")
        require(row_id in doc_text, f"v0.6.153 review document missing reviewed row id: {row_id}")

    for column in REQUIRED_COLUMNS:
        require(column in matrix_text, f"Control Matrix missing reviewed column: {column}")
    ...
    for phrase in [
        ...
        require(phrase in matrix_text, f"Control Matrix missing reviewed safeguard: {phrase}")
    ...
    for phrase in [
        ...
        require(phrase in v06152, f"v0.6.152 candidate missing required phrase: {phrase}")
    ...
    for phrase in [
        ...
        require(phrase not in lower_doc, f"v0.6.153 document must not include affirmative forbidden phrase: {phrase}")

    print("v0.6.153 Control Matrix review and decision tests passed.")
    return 0
