...
DOC = ROOT / "docs/162-v0686-narrow-public-safe-aaef-main-handback-preparation-planning.md"
V0685_DOC = ROOT / "docs/161-v0685-applied-evidence-handback-preparation-decision.md"
...
def main() -> int:
    require(V0685_DOC.exists(), "v0.6.85 handback preparation decision document must exist")
    ...
    for flag in REQUIRED_DOC_FLAGS:
        require(flag in doc_text, f"v0.6.86 document missing planning flag: {flag}")

    for phrase in [
        "public-safe source boundary",
        "evidence/interface-only scope",
        "AAEF five-questions alignment",
        "Model-output-is-not-authority",
        "Validator-output-is-not-authority",
        "Non-execution evidence",
        "Static public sample boundary",
        "Patent-sensitive exclusion",
        "Private artifact exclusion",
        "Customer and credential exclusion",
        "Scanner output exclusion",
        "Delivery exclusion",
        "Claim boundary",
        "AAEF main submission boundary",
    ]:
        require(phrase.lower() in doc_text.lower(), f"v0.6.86 document missing preparation control: {phrase}")
    ...
    for phrase in [
        ...
        require(phrase in doc_text, f"v0.6.86 document missing retained lesson or boundary phrase: {phrase}")
    ...
    for phrase in [
        ...
        require(phrase in doc_text, f"v0.6.86 document missing unresolved decision phrase: {phrase}")

    for category in EXPECTED_CATEGORIES:
        require(f"`{category}`" in doc_text, f"v0.6.86 document missing retained category: {category}")
        require((FIXTURE_ROOT / category).exists(), f"fixture directory missing: {category}")

    for field in REQUIRED_METADATA_FIELDS:
        require(field in doc_text, f"v0.6.86 document missing retained metadata field: {field}")

    for flag in REQUIRED_BOUNDARY_FLAGS:
        require(f"{flag} = false" in doc_text, f"v0.6.86 document missing retained false boundary flag: {flag}")

    v0685 = V0685_DOC.read_text(encoding="utf-8")
    for phrase in [
        ...
        require(phrase in v0685, f"v0.6.85 baseline missing required phrase: {phrase}")

    v0684 = V0684_DOC.read_text(encoding="utf-8")
    require("public_safe_evidence_interface_boundary_lessons_close_ready = true" in v0684, "v0.6.84 must close eligible lesson family")
    ...
    for category in EXPECTED_CATEGORIES:
        ...
        require("expected_failure_category" not in metadata, f"v0.6.86 must not add metadata-level failure category: {category}")
    ...
    for phrase in [
        ...
        require(phrase not in lower_doc, f"v0.6.86 document must not include affirmative overclaim phrase: {phrase}")

    print("v0.6.86 narrow public-safe AAEF main handback preparation planning tests passed.")
    return 0
