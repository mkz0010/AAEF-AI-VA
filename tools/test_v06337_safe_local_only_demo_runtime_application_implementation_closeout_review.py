...
def test_v06337_primary_files() -> None:
    assert_tokens(DOC, REQUIRED)
    assert_tokens(ADR, [
        "ADR-0412",
        "Status: completed closeout",
        "Close the safe local-only demo runtime application implementation candidate track while preserving runtime-applied false.",
    ])
    assert_tokens(ISSUE, [
        ...
        "Proceed to v0.6.338 Next Work Selection Using Risk-Tiered Granularity",
    ])

def test_v06337_index_files() -> None:
...
def test_v06337_registered_in_run_all() -> None:
    assert_tokens(RUN_ALL, ["tools/test_v06337_safe_local_only_demo_runtime_application_implementation_closeout_review.py"])

def test_v06337_avoids_raw_forbidden_claim_phrases() -> None:
...
def main() -> None:
    test_v06337_primary_files()
    test_v06337_index_files()
    test_v06337_registered_in_run_all()
    test_v06337_avoids_raw_forbidden_claim_phrases()
    print("v0.6.337 safe local-only demo runtime application implementation closeout review checks passed")
