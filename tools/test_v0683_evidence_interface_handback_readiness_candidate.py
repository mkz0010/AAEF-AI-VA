...
DOC = ROOT / "docs/159-v0683-evidence-interface-handback-readiness-candidate.md"
...
def main() -> int:
    ...
    for flag in REQUIRED_DOC_FLAGS:
        require(flag in doc_text, f"v0.6.83 document missing candidate flag: {flag}")
    ...
    for section in [
        ...
        require(section in doc_text, f"v0.6.83 document missing candidate section: {section}")
    ...
    for lesson in [
        ...
        require(lesson in doc_text, f"v0.6.83 document missing candidate lesson: {lesson}")
    ...
    for question in [
        ...
        require(question in doc_text, f"v0.6.83 document missing AAEF question: {question}")
    ...
    for phrase in [
        ...
        "It must not claim that the public sample proves runtime execution",
    ]:
        require(phrase in doc_text, f"v0.6.83 document missing readiness phrase: {phrase}")
    ...
    for phrase in [
        ...
        require(phrase in doc_text, f"v0.6.83 document missing candidate acceptance check: {phrase}")
    ...
    for phrase in [
        ...
        require(phrase in doc_text, f"v0.6.83 document missing retained validator boundary phrase: {phrase}")

    for category in EXPECTED_CATEGORIES:
        require(f"`{category}`" in doc_text, f"v0.6.83 document missing retained category: {category}")
        require((FIXTURE_ROOT / category).exists(), f"fixture directory missing: {category}")

    for field in REQUIRED_METADATA_FIELDS:
        require(field in doc_text, f"v0.6.83 document missing retained metadata field: {field}")

    for flag in REQUIRED_BOUNDARY_FLAGS:
        require(f"{flag} = false" in doc_text, f"v0.6.83 document missing retained false boundary flag: {flag}")
    ...
    for phrase in [
        ...
        require(phrase in doc_text, f"v0.6.83 document missing unresolved decision phrase: {phrase}")

    v0682_text = V0682_DOC.read_text(encoding="utf-8")
    require(
        "evidence_interface_handback_readiness_planning_completed = true" in v0682_text,
        "v0.6.82 handback readiness planning must be completed",
    )
    require(
        "evidence_interface_handback_readiness_candidate_may_be_considered = true" in v0682_text,
        "v0.6.82 must allow readiness candidate consideration",
    )
    require(
        "aaef_main_handback_prepared = false" in v0682_text,
        "v0.6.82 must not prepare AAEF main handback",
    )
    require(
        "aaef_main_issue_opened = false" in v0682_text,
        "v0.6.82 must not open AAEF main issue",
    )
    require(
        "aaef_main_pull_request_opened = false" in v0682_text,
        "v0.6.82 must not open AAEF main pull request",
    )
    require("validator_behavior_modified = false" in v0682_text, "v0.6.82 must preserve validator behavior boundary")
    ...
    for category in EXPECTED_CATEGORIES:
        ...
        require("expected_failure_category" not in metadata, f"v0.6.83 must not add metadata-level failure category: {category}")
    ...
    forbidden_affirmative_phrases = [
        ...
        "this readiness candidate provides certification",
        "this readiness candidate is legally sufficient",
        "this readiness candidate provides an audit opinion",
        "this readiness candidate prepares aaef main handback material",
        "this candidate opens an aaef main issue",
        "scanner execution is authorized by this readiness candidate",
        "runtime execution is authorized by this readiness candidate",
    ]
    for phrase in forbidden_affirmative_phrases:
        require(phrase not in lower_doc, f"v0.6.83 document must not include affirmative overclaim phrase: {phrase}")

    print("v0.6.83 evidence-interface handback readiness candidate tests passed.")
    return 0
