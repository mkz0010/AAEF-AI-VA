...
DOC = ROOT / "docs/187-v06111-narrow-public-safe-aaef-main-handback-workflow-selection-or-exact-text-preparation-decision.md"
V06110_DOC = ROOT / "docs/186-v06110-narrow-public-safe-aaef-main-handback-submission-workflow-planning.md"
V06109_DOC = ROOT / "docs/185-v06109-applied-evidence-handback-submission-or-pause-decision.md"
...
def main() -> int:
    for path in [DOC, V06110_DOC, V06109_DOC, V0655_DOC, INDEX, VALIDATOR]:
    ...
    for phrase in REQUIRED_PHRASES + REQUIRED_FLAGS:
        require(phrase in doc_text, f"v0.6.111 document missing required text: {phrase}")

    for category in EXPECTED_CATEGORIES:
        require(category in doc_text, f"v0.6.111 document missing retained category: {category}")
        ...
        require("expected_failure_category" not in metadata, f"v0.6.111 must not add metadata-level failure category: {category}")
    ...
    for phrase in [
        ...
        require(phrase in v06110, f"v0.6.110 baseline missing required phrase: {phrase}")

    v06109 = V06109_DOC.read_text(encoding="utf-8")
    require(
        "aaef_main_handback_submission_workflow_planning_authorized_for_next_checkpoint = true" in v06109,
        "v0.6.109 must authorize submission workflow planning",
    )
    ...
    for phrase in [
        ...
        require(phrase not in lower_doc, f"v0.6.111 document must not include affirmative overclaim phrase: {phrase}")

    print("v0.6.111 narrow public-safe AAEF main handback workflow selection or exact text preparation decision tests passed.")
    return 0
