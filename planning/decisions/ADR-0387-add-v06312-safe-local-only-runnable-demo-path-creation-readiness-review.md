# ADR-0387: Review Safe Local-Only Runnable Demo Path Creation Readiness
...
v0.6.311 accepted the Safe Local-Only Runnable Demo Path as a documentation-level path.

## Decision

Record that the project is ready to prepare a Safe Local-Only Runnable Demo Path Creation Candidate, while keeping runnable path creation, runtime application, runtime readiness, scanner readiness, execution authorization, and real execution deferred.
...
The project should next create a Safe Local-Only Runnable Demo Path Creation Candidate. The candidate must remain mock-first, localhost-only, blocked-by-default, and private-not-in-git oriented.
