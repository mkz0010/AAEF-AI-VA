# ADR-0395: Review Safe Local-Only Runnable Demo Public Positioning
...
v0.6.319 accepted the local reviewer runbook for the safe local-only runnable demo.

## Decision

Record that the safe local-only runnable demo needs a public positioning candidate before any public wording integration. The positioning must preserve mock-first, localhost-only, local reviewer walkthrough scope and avoid scanner/readiness/authorization claims.
...
The project should next create a public positioning candidate. Publication, customer demonstration, runtime application, runtime readiness, scanner readiness, execution authorization, and real execution remain deferred.
